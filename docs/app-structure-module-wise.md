...
- `tools/pdf_processor/`: Python PDF processing microservice used by backend document/template workflows.
...
| `tools/pdf_processor/` | Python FastAPI/Uvicorn PDF tool packaged separately with its own Dockerfile and requirements. |
...
| `docker-compose.yml` | Production-like composition: Postgres, combined app container, PDF tool. |
| `docker-compose.dev.yml` | Dev composition: separate backend, frontend, PDF tool services. |
...
## 6. PDF Processor Service

| Path | Purpose |
| --- | --- |
| `tools/pdf_processor/app.py` | Python API service for PDF processing. |
| `tools/pdf_processor/requirements.txt` | Python dependencies. |
| `tools/pdf_processor/Dockerfile` | Container image for the PDF service. |
| `tools/pdf_processor/start.bat`, `start.sh` | Local startup helpers. |
| `tools/pdf_processor/test_client.py` | Test client for the PDF processor. |

Docker compose wires this service as `pdf-tool`. The backend uses `PDF_TOOL_URL` to communicate with it.

Note: `docker-compose.yml` sets `PDF_TOOL_URL=http://pdf-tool:8002` while the service exposes port `8001`; this should be verified before production deployment.
...
- `pdf-tool`: PDF processor container, exposed on `8001`.
...
- `pdf-tool`: reloadable Uvicorn PDF service on `8001`.
...
6. `deployment-runbook.md`: Docker, environment variables, migrations, uploads, PDF tool, backups.
