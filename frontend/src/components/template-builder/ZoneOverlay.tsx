...
import type { TemplateZone, ZoneBounds, ZoneType } from '../../types/template.types';
...
const ZoneOverlay = ({
    ...
    const [resizeStart, setResizeStart] = useState<{ mouse: { x: number; y: number }; bounds: ZoneBounds } | null>(null);

    // Drag state
    const [dragStart, setDragStart] = useState<{ mouse: { x: number; y: number }; bounds: ZoneBounds } | null>(null);
    ...
    const handleMouseMove = useCallback((e: React.MouseEvent) => {
        ...
        if (mode === 'resizing' && resizeStart && activeHandle && selectedZoneId) {
            ...
            switch (activeHandle) {
                case 'nw':
                    newBounds = { x: bounds.x + dx, y: bounds.y + dy, w: bounds.w - dx, h: bounds.h - dy };
                    break;
                case 'n':
                    newBounds = { ...bounds, y: bounds.y + dy, h: bounds.h - dy };
                    break;
                case 'ne':
                    newBounds = { ...bounds, y: bounds.y + dy, w: bounds.w + dx, h: bounds.h - dy };
                    break;
                case 'e':
                ...
                case 's':
                    ...
                    break;
                case 'sw':
                    newBounds = { x: bounds.x + dx, y: bounds.y, w: bounds.w - dx, h: bounds.h + dy };
                    break;
                case 'w':
                    newBounds = { x: bounds.x + dx, y: bounds.y, w: bounds.w - dx, h: bounds.h };
                    break;
            }

            // Enforce minimum size and positive values
            if (newBounds.w >= MIN_ZONE_SIZE && newBounds.h >= MIN_ZONE_SIZE &&
                newBounds.x >= 0 && newBounds.y >= 0) {
                onZonesChange(zones.map(z => z.id === selectedZoneId ? { ...z, bounds: newBounds } : z));
            }
            return;
        }
        ...
        if (mode === 'dragging' && dragStart && selectedZoneId) {
            ...
            const newY = Math.max(0, Math.min(pageHeight - bounds.h, bounds.y + dy));

            onZonesChange(zones.map(z => z.id === selectedZoneId ? { ...z, bounds: { ...bounds, x: newX, y: newY } } : z));
        }
    }, [mode, drawStart, resizeStart, activeHandle, selectedZoneId, dragStart, getMousePosition, zones, onZonesChange, pageWidth, pageHeight]);

    // Finish any interaction
    const handleMouseUp = useCallback(() => {
        // Finish drawing
        if (mode === 'drawing' && currentBounds && currentBounds.w >= MIN_ZONE_SIZE && currentBounds.h >= MIN_ZONE_SIZE) {
    ...
    const handleResizeStart = useCallback((e: React.MouseEvent, zoneId: string, handle: ResizeHandle) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const getZoneIcon = (type: ZoneType): string => {
        const icons: Record<ZoneType, string> = {
            header: '📋', table: '📊', label_value: '🏷️', multiline: '📝',
            list: '📑', date_field: '📅', amount_field: '💰',
        };
        return icons[type] || '📋';
    };
    ...
    const renderResizeHandles = (zone: TemplateZone) => {
        if (zone.id !== selectedZoneId || isDrawing || mode === 'drawing') return null;
    ...
    return (
        ...
        >
                ...
                return (
                    <g key={zone.id}>
                        ...
                        <rect
                            ...
                            rx={4}
                            style={{ cursor: isDrawing ? 'crosshair' : (mode === 'dragging' ? 'grabbing' : 'grab') }}
            ...
            {mode === 'drawing' && currentBounds && currentBounds.w > 0 && currentBounds.h > 0 && (
                <rect
                    x={currentBounds.x}
                    y={currentBounds.y}
