...
const GanttChart: React.FC<GanttChartProps> = ({ data }) => {
    ...
    // 1. Prepare Data
    const activities = useMemo(() => data.filter(a => {
    ...
    const timeline = useMemo(() => {
        ...
        activities.forEach(a => {
            const start = new Date(a.schedule?.earlyStart || a.startDatePlanned).getTime();
            const finish = new Date(a.schedule?.earlyFinish || a.finishDatePlanned).getTime();
        ...
        if (viewMode === 'Day') {
            ...
            for (let i = 0; i < days; i++) {
                subHeaders.push({ label: String(startCopy.getDate()), width: cellWidth });
            ...
            const weeks = Math.ceil((end.getTime() - start.getTime()) / (1000 * 3600 * 24 * 7));
            ...
            while (curr < end) {
                subHeaders.push({ label: curr.toLocaleDateString('default', { month: 'short', year: '2-digit' }), width: cellWidth });
            ...
            while (curr < end) {
                ...
                subHeaders.push({ label: `Q${q} '${curr.getFullYear().toString().slice(2)}`, width: cellWidth });
    ...
    if (!timeline) {
        return <div className="p-10 text-center text-gray-500">No scheduled activities to display. Import schedule or run CPM.</div>;
    }
    ...
    const getPosition = (dateStr: string) => {
        ...
        const msPerPixel = (viewMode === 'Day' ? (1000 * 3600 * 24) :
            viewMode === 'Week' ? (1000 * 3600 * 24 * 7) :
                viewMode === 'Month' ? (1000 * 3600 * 24 * 30) : // approx
    ...
    const Row = ({ index, style }: { index: number; style: CSSProperties }) => {
        ...
        const combinedName = act.wbsNode ? `${act.wbsNode.name} - ${act.activityName}` : act.activityName;

        return (
            <div style={style} className="flex border-b border-gray-100 bg-white hover:bg-gray-50 group">
                {/* Sticky Sidebar */}
                <div
                    className="sticky left-0 z-10 bg-white border-r border-gray-200 flex items-center px-4 text-sm truncate shrink-0 group-hover:bg-gray-50"
                    style={{ width: sidebarWidth, height: '100%' }}
                    title={combinedName}
                >
                    <span className="font-mono text-xs text-gray-500 mr-2 shrink-0">{act.activityCode}</span>
                    <span className="truncate">{combinedName}</span>
                </div>

                {/* Timeline */}
                <div className="relative flex-1 h-full" style={{ minWidth: totalWidth }}>
                    {/* Horizontal Grid Lines (Optional) */}

                    {/* Bar */}
                    <div
                        className={clsx("absolute top-2.5 h-5 rounded-sm text-[10px] flex items-center px-1 text-white overflow-hidden whitespace-nowrap shadow-sm transition-all",
                            isCritical ? 'bg-red-500' : 'bg-blue-500 opacity-80'
                        )}
                        style={{ left, width }}
    ...
    return (
        <div className="flex-1 h-full w-full overflow-hidden bg-white flex flex-col border border-gray-200 min-h-0">
            {/* Toolbar */}
            <div className="h-10 bg-white border-b border-gray-200 flex items-center px-4 justify-between shrink-0">
                <span className="font-semibold text-gray-700">Gantt View: {viewMode}</span>
                <div className="flex space-x-2">
                    <button onClick={zoomOut} className="p-1 hover:bg-gray-100 rounded border border-gray-200" title="Zoom Out"><ZoomOut className="w-4 h-4 text-gray-600" /></button>
                    <button onClick={zoomIn} className="p-1 hover:bg-gray-100 rounded border border-gray-200" title="Zoom In"><ZoomIn className="w-4 h-4 text-gray-600" /></button>
                </div>
            </div>

            {/* Header */}
            <div ref={headerRef} className="flex border-b border-gray-200 bg-gray-50 overflow-hidden shrink-0" style={{ height: headerHeight }}>
                <div className="sticky left-0 z-20 bg-gray-50 border-r border-gray-200 flex items-center px-4 font-semibold text-xs text-gray-500 shrink-0" style={{ width: sidebarWidth }}>
                    Activity
                </div>
                <div className="flex" style={{ width: totalWidth }}>
                    {subHeaders.map((h, i) => (
                        <div key={i} className="flex-shrink-0 border-r border-gray-200 flex flex-col items-center justify-center text-[10px] text-gray-500 font-medium" style={{ width: h.width }}>
                            {h.label}
                        </div>
                    ))}
                </div>
