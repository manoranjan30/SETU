...
const TemplateImportModal: React.FC<Props> = ({ isOpen, onClose, projectId, onSuccess }) => {
    ...
    const handleApply = async () => {
        ...
        if (!confirm('This will create WBS nodes from the selected template. Proceed?')) return;

        setApplying(true);
        try {
            await api.post(`/projects/${projectId}/wbs/templates/${selectedId}/apply`);
                        ...
                        <div className="space-y-3">
                            {templates.map(tpl => (
                                <div
                                    ...
                                    className={`p-4 border rounded-lg cursor-pointer transition-all flex items-start gap-3 ${selectedId === tpl.id ? 'border-blue-600 bg-blue-50 ring-1 ring-blue-600' : 'hover:border-gray-300'}`}
                                >
                                    <div className={`mt-1 w-4 h-4 rounded-full border flex items-center justify-center ${selectedId === tpl.id ? 'border-blue-600 bg-blue-600' : 'border-gray-400'}`}>
                                        {selectedId === tpl.id && <Check className="w-3 h-3 text-white" />}
                                    </div>
                                    <div>
                                        <h4 className="font-semibold text-gray-900">{tpl.templateName}</h4>
