...
const BoqTreePanel: React.FC<Props> = ({
  projectId,
  selectedIds,
  onMultiSelect,
  refreshTrigger,
  ...
  const treeData = useMemo(() => {
    ...
    const sorted = [...items].sort((a, b) =>
      (a.boqCode || "").localeCompare(b.boqCode || "", undefined, {
        numeric: true,
      }),
    );
    ...
    sorted.forEach((item) => {
      ...
      const linkedInfo = item.mappedActivities
        ? ` 🔗 ${item.mappedActivities}`
        : "";
      ...
      if ((item as any).subItems) {
        (item as any).subItems.forEach((sub: any) => {
          ...
          const subNode: TreeNodeData = {
            ...
            icon: (
              <div
                className={`w-2 h-2 rounded-full ${effectiveStatus === "MAPPED" ? "bg-success" : "bg-orange-300"}`}
              />
            ),
          };
          ...
          if (sub.measurements) {
            sub.measurements.forEach((meas: any) => {
              ...
              const measNode: TreeNodeData = {
                ...
                icon: (
                  <div
                    className={`text-[10px] ${measStatus === "MAPPED" ? "text-success font-bold" : "text-text-disabled"}`}
                  >
                    #
                  </div>
                ),
  ...
  return (
    <div className="flex flex-col h-full bg-surface-base">
      <div className="p-2 border-b bg-surface-card">
        <input
          type="text"
        ...
        <Tree
          ...
          getItemClassName={(node) => {
            ...
            if (item.mappingStatus === "MAPPED")
              return "bg-success-muted border-green-100 hover:bg-green-100";
            if (item.mappingStatus === "PARTIAL")
