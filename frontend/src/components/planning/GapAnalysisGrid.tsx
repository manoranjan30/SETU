...
export const GapAnalysisGrid: React.FC<GapAnalysisGridProps> = ({
  projectId,
}) => {
  ...
  const fetchGapAnalysis = async () => {
    ...
    try {
      ...
      // Calculate stats
      const s = response.data.reduce(
        (acc: any, row: ActivityGapData) => {
          acc.total++;
    ...
    () => [
      ...
      },
      {
        ...
        cellRenderer: (p: any) => {
          ...
          if (status === "READY")
            return (
              <span className={`${baseClass} bg-info-muted text-blue-800`}>
                Ready
              </span>
            );
          if (status === "MISSING_BOQ")
            return (
              <span className={`${baseClass} bg-orange-100 text-orange-800`}>
                Missing BOQ
              </span>
            );
          if (status === "CRITICAL_UNLINKED_EXECUTION")
            return (
  ...
  const StatCard = ({
    ...
    <div className="bg-surface-card p-4 rounded-lg shadow-sm border border-border-default">
  ...
  return (
    <div className="space-y-4 h-full flex flex-col">
      ...
      <div
        className="ag-theme-alpine ag-theme-setu flex-1 w-full"
        style={{ minHeight: 400 }}
      >
        <AgGridReact
          rowData={rowData}
