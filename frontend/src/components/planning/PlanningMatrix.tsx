...
interface Stats {
    ...
    schedule: { total: number, linked: number, unlinked: number, coverage: number };
}
...
const PlanningMatrix: React.FC = () => {
    ...
    // View State
    const [activeTab, setActiveTab] = useState<'DASHBOARD' | 'MATRIX' | 'DISTRIBUTION' | 'DISTRIBUTED_VIEW'>('DASHBOARD');
    const [dashboardSubTab, setDashboardSubTab] = useState<'MATRIX' | 'UNLINKED_ACT' | 'UNMAPPED_BOQ' | 'GAP_ANALYSIS'>('GAP_ANALYSIS');
    ...
    const StatusCard = ({ title, value, subtext, color }: any) => (
        <div className={`bg-white p-4 rounded-lg shadow-sm border border-l-4 border-${color}-500`}>
            ...
            <div className="flex justify-between items-center bg-white p-2 rounded shadow-sm border border-gray-200">
                <div className="flex gap-1">
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded flex items-center gap-2 transition-colors ${activeTab === 'DASHBOARD' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-50'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded flex items-center gap-2 transition-colors ${activeTab === 'MATRIX' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-50'
                            }`}
                    >
                    ...
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded flex items-center gap-2 transition-colors ${activeTab === 'DISTRIBUTED_VIEW' ? 'bg-blue-100 text-blue-700' : 'text-gray-600 hover:bg-gray-50'
                            }`}
                    >
            ...
            <div className="flex-1 overflow-hidden relative flex flex-col min-h-0">
                ...
                {activeTab === 'MATRIX' && (
                    <div className="flex-1 p-4 flex flex-col h-full min-h-0 w-full">
                        <div className="flex-1 relative min-h-0 w-full bg-white rounded-lg shadow-sm border border-gray-200">
                        ...
                        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden min-h-[500px] flex flex-col">
                            <div className="flex border-b">
                                <button
                                    ...
                                    className={`px-6 py-3 text-sm font-medium border-r ${dashboardSubTab === 'GAP_ANALYSIS' ? 'border-b-2 border-purple-600 text-purple-600 bg-purple-50' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    Gap Analysis
                                </button>
                                <button
                                    onClick={() => setDashboardSubTab('MATRIX')}
                                    className={`px-6 py-3 text-sm font-medium ${dashboardSubTab === 'MATRIX' ? 'border-b-2 border-blue-600 text-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    Map Status ({matrix.length})
                                </button>
                                <button
                                    onClick={() => setDashboardSubTab('UNLINKED_ACT')}
                                    className={`px-6 py-3 text-sm font-medium flex items-center gap-2 ${dashboardSubTab === 'UNLINKED_ACT' ? 'border-b-2 border-red-600 text-red-600 bg-red-50' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                ...
                                <button
                                    ...
                                    className={`px-6 py-3 text-sm font-medium flex items-center gap-2 ${dashboardSubTab === 'UNMAPPED_BOQ' ? 'border-b-2 border-orange-600 text-orange-600 bg-orange-50' : 'text-gray-500 hover:text-gray-700'}`}
                                >
                                    ...
                                    <>
                                            ...
                                            <table className="min-w-full divide-y divide-gray-200">
                                                <thead className="bg-gray-50">
                                                    <tr>
                                                        ...
                                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Linked Activity</th>
                                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Mapped Qty</th>
                                                ...
                                                <tbody className="bg-white divide-y divide-gray-200">
                                                    {matrix.map(row => (
                                                        <tr key={row.id}>
                                                            <td className="px-6 py-4 text-sm text-gray-900">{row.boqItem?.description}</td>
                                                            <td className="px-6 py-4 text-sm text-gray-900">{row.activity?.activityName} <span className='text-xs text-gray-400'>({row.activity?.activityCode})</span></td>
                                                            <td className="px-6 py-4 text-sm font-mono">{row.plannedQuantity} {row.boqItem?.uom}</td>
                                                            <td className="px-6 py-4 text-xs text-gray-500">{row.mappingType}</td>
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
                                        ...
                                        {dashboardSubTab === 'UNLINKED_ACT' && (
                                            <div className="flex flex-col h-full">
                                                <div className="p-4 bg-red-50 text-red-700 text-sm flex justify-between items-center">
                                                    <span>Found <b>{unlinkedActivities.length}</b> activities with no budget/scope assigned. This is a risk.</span>
                                                    <button
                                                        onClick={() => navigate({ search: '?view=mapper' })}
                                                        className="bg-white border border-red-200 text-red-700 px-3 py-1 rounded shadow-sm hover:bg-red-100 text-xs font-bold"
                                                    >
                                                        Go to Mapper to Fix
                                                    </button>
                                                </div>
                                                <table className="min-w-full divide-y divide-gray-200">
                                                    <thead className="bg-gray-50">
                                                        <tr>
                                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Activity Code</th>
                                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Activity Name</th>
                                                    ...
                                                    <tbody className="bg-white divide-y divide-gray-200">
                                                        {unlinkedActivities.map(act => (
                                                            <tr key={act.id}>
                                                                <td className="px-6 py-4 text-sm font-mono text-gray-600">{act.activityCode}</td>
                                                                <td className="px-6 py-4 text-sm font-medium text-gray-900">{act.activityName}</td>
                                                                <td className="px-6 py-4 text-sm text-gray-500">{new Date(act.startDatePlanned).toLocaleDateString()}</td>
                                                                <td className="px-6 py-4">
                                                                    <span className="px-2 py-1 bg-red-100 text-red-800 text-xs rounded-full">No Scope</span>
                                                                </td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
                                        ...
                                        {dashboardSubTab === 'UNMAPPED_BOQ' && (
                                            <div className="flex flex-col h-full">
                                                <div className="p-4 bg-orange-50 text-orange-700 text-sm flex justify-between items-center">
                                                    <span>Found <b>{unmappedBoq.length}</b> BOQ items not fully distributed to schedule. Execution is missing scope.</span>
                                                    <button
                                                        onClick={() => navigate({ search: '?view=mapper' })}
                                                        className="bg-white border border-orange-200 text-orange-700 px-3 py-1 rounded shadow-sm hover:bg-orange-100 text-xs font-bold"
                                                    >
                                                        Go to Mapper to Fix
                                                    </button>
                                                </div>
                                                    ...
                                                    <tbody className="bg-white divide-y divide-gray-200">
                                                        {unmappedBoq.map(item => (
                                                            <tr key={item.id}>
                                                                ...
                                                                <td className="px-6 py-4 text-sm font-mono">{item.qty} {item.uom}</td>
                                                                <td className="px-6 py-4">
                                                                    <span className={`px-2 py-1 text-xs rounded-full ${item.mappingStatus === 'PARTIAL' ? 'bg-orange-100 text-orange-800' : 'bg-red-100 text-red-800'}`}>
                                                                        {item.mappingStatus}
                                                                    </span>
                                                                </td>
                                                            </tr>
