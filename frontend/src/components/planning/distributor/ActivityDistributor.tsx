...
const ActivityDistributor: React.FC = () => {
  ...
  const TreeNode = ({ node, level = 0 }: { node: any; level?: number }) => {
    ...
    return (
      <div className="select-none">
        <div
          className={`flex items-center hover:bg-surface-base py-1 ${isActivity ? "ml-6" : ""}`}
          style={{ paddingLeft: `${level * 16}px` }}
        >
          {!isActivity && (
            <button
              onClick={() => setExpanded(!expanded)}
              className="mr-1 text-text-disabled"
            >
              {expanded ? (
                <ChevronDown size={14} />
              ) : (
          ...
          {isActivity ? (
            <button
              onClick={() => toggleActivity(node.id)}
              className={`mr-2 ${selectedActivities.has(node.id) ? "text-primary" : "text-gray-300"}`}
            >
              {selectedActivities.has(node.id) ? (
                <CheckSquare size={16} />
              ) : (
                <Square size={16} />
              )}
            ...
            <span className="mr-2 text-text-disabled">
              <Square size={16} className="opacity-0" />
            </span> // Placeholder alignment
          )}

          <div className="flex-1">
            <span
              className={`text-sm ${isActivity ? "text-text-secondary" : "text-text-primary font-semibold"}`}
            >
              {isActivity ? node.activityName : node.wbsName}
            </span>
            {isActivity && (
              <span className="ml-2 text-xs text-text-disabled font-mono">
                {node.activityCode}
              </span>
            )}
          </div>
  ...
  const TargetNode = ({ node, level = 0 }: { node: any; level?: number }) => {
    ...
    return (
      <div>
        <div
          className="flex items-center hover:bg-surface-base py-1"
          style={{ paddingLeft: `${level * 16}px` }}
        >
          ...
          <button
            ...
            className={`mr-2 ${selectedTargets.has(node.id) ? "text-success" : "text-gray-300"}`}
          >
            {selectedTargets.has(node.id) ? (
              <CheckSquare size={16} />
            ) : (
              <Square size={16} />
            )}
          ...
          <div className="flex-1">
            ...
            <span className="ml-2 text-xs text-text-disabled uppercase">
              {node.type}
            </span>
          </div>
        </div>
        {expanded && node.children && (
          <div>
            {node.children.map((c: any) => (
              <TargetNode key={c.id} node={c} level={level + 1} />
            ))}
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col h-full bg-surface-card rounded-lg shadow-sm border border-border-default">
      {/* Header */}
      <div className="p-4 border-b border-border-subtle flex justify-between items-center bg-surface-base">
        <div>
          <h2 className="text-lg font-bold text-gray-800">
            Schedule Distributor
          </h2>
          <p className="text-xs text-text-muted">
            Clone master activities to other EPS nodes (Towers/Blocks).
          </p>
        </div>
        <div className="flex items-center gap-4">
          {result && (
            <span
              className={`text-sm ${result.created > 0 ? "text-success" : "text-text-muted"}`}
            >
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Source */}
        <div className="flex-1 border-r border-border-default flex flex-col min-w-[300px]">
          <div className="p-2 bg-surface-raised text-xs font-bold text-text-muted uppercase tracking-wider">
        ...
        {/* Arrow */}
        <div className="w-12 bg-surface-base flex items-center justify-center border-r border-border-default">
        ...
        <div className="flex-1 flex flex-col min-w-[300px]">
          ...
          <div className="flex-1 overflow-auto p-2">
            {targetEps.map((node) => (
              <TargetNode key={node.id} node={node} />
            ))}
          </div>
        </div>
      </div>
