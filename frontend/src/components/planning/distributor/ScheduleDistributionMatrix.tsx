...
const ScheduleDistributionMatrix: React.FC = () => {
  ...
  const fetchStructure = async () => {
    ...
    try {
      ...
      setExpandedRows((prev) =>
        prev.length === 0 ? rootNodes.map((n: any) => n.id) : prev,
      );
  ...
  const Row = ({ node, level = 0 }: { node: any; level?: number }) => {
    ...
    return (
      <>
        <tr
          className={`hover:bg-primary-muted transition-colors group border-b border-border-subtle ${isActivity ? "bg-surface-card" : "bg-surface-base/50"}`}
        >
          {/* WBS Name Column */}
          <td className="sticky left-0 z-20 border-r border-border-default p-0 shadow-[2px_0_5px_rgba(0,0,0,0.05)] bg-inherit align-middle">
            <div
              className="flex items-center h-10 pr-2 border-l-4 border-transparent hover:border-blue-400 transition-colors"
              style={{ paddingLeft }}
            >
              {!isActivity && (
                <button
              ...
              {isActivity ? (
                ...
                <span
                  className="text-xs font-bold text-text-muted uppercase tracking-wide truncate"
                  title={node.wbsName}
                >
                  {node.wbsName}
                </span>
          ...
          {leafColumns.map((target) => {
            ...
            const btnClass = isDistributed
              ? "bg-primary-muted/50 hover:bg-success-muted group-hover:bg-info-muted"
              : "text-gray-200 hover:text-text-disabled group-hover:bg-surface-card";
                ...
                >
                  {isProcessing ? (
                    ...
                    <CheckSquare
                      ...
                      fill="currentColor"
                      fillOpacity={isActivity ? 0.1 : 0.05}
                    />
                  ) : (
                    <Square
                      size={18}
  ...
  return (
    <div className="flex flex-col h-full w-full bg-surface-card rounded-lg shadow-sm border border-border-default overflow-hidden relative min-h-0">
      <div className="p-4 border-b border-border-default bg-surface-card flex justify-between items-center z-20 flex-shrink-0">
        <div>
          <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
            Schedule Mapper
            <span className="text-xs font-normal text-white px-2 py-1 bg-primary rounded-full">
              Hierarchical
            </span>
          </h2>
        ...
        <div className="flex items-center gap-4 text-sm text-text-secondary">
          ...
          <label className="flex items-center gap-2 cursor-pointer bg-primary-muted px-3 py-1 rounded border border-blue-100 hover:bg-info-muted transition-colors">
            <input
              type="checkbox"
          ...
          <div className="flex items-center gap-2">
            <span className="flex items-center gap-1">
              <CheckSquare size={14} className="text-primary" /> Linked
            </span>
            <span className="flex items-center gap-1">
              <Square size={14} className="text-gray-300" /> Unlinked
            </span>
          </div>
        </div>
      </div>

      <div className="flex-1 overflow-auto relative bg-surface-base/50 min-h-0 flex">
        ...
        {loading && (
          <div className="absolute inset-0 bg-surface-card/80 z-50 flex items-center justify-center">
        ...
        {!loading && targetEpsRoot.length > 0 && sourceWbs.length > 0 && (
          <table className="min-w-full border-separate border-spacing-0 bg-surface-card">
            <thead className="sticky top-0 z-30 bg-surface-base shadow-sm">
              {headerRows.map((row, rowIndex) => (
                <tr key={rowIndex}>
                  {rowIndex === 0 && (
                    <th
                      rowSpan={headerRows.length}
                      className="sticky left-0 z-40 bg-surface-raised border-r border-b border-border-strong p-3 text-left w-[300px] min-w-[300px] shadow-[2px_0_5px_rgba(0,0,0,0.05)] align-bottom"
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-text-disabled uppercase tracking-wider">
                          Master Schedule
                        </span>
                        <span className="text-[10px] text-text-disabled bg-gray-200 px-1 rounded">
                          {sourceWbs.length} Roots
                        </span>
                      </div>
                    </th>
                  )}
                  {row.map((node) => (
                    <MatrixHeader
                      ...
                      stopType={restrictToFloor ? "FLOOR" : undefined}
                      isLocked={
                        restrictToFloor &&
                        node.type &&
                        node.type.toUpperCase() === "FLOOR"
                      }
                    />
                  ))}
                </tr>
