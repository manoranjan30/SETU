...
import { ChevronRight, ChevronDown, Search, Check, Calendar } from 'lucide-react';
...
const WbsActivityTreeSelector: React.FC<WbsActivityTreeSelectorProps> = ({
    ...
    const renderNode = (node: TreeNode, level: number = 0) => {
        ...
        const isSelected = node.type === 'ACTIVITY' && selectedIds.includes(node.id);
        const hasChildren = node.children.length > 0;

        return (
            <div key={node.key} style={{ marginLeft: level * 0 }}>
                <div className="flex items-center py-1 hover:bg-gray-50 rounded text-sm">
                    {/* Indent / Expand Icon */}
                    <div
                        className="w-6 h-6 flex items-center justify-center cursor-pointer text-gray-400 mr-1"
                    ...
                    {node.type === 'ACTIVITY' ? (
                        <div
                            className={`w-4 h-4 mr-2 border rounded flex items-center justify-center cursor-pointer ${isSelected ? 'bg-blue-600 border-blue-600' : 'border-gray-300'}`}
                    ...
                    {/* Content */}
                    <div
                        className={`flex-1 cursor-pointer ${node.type === 'WBS' ? 'font-semibold text-gray-700' : 'text-gray-600'}`}
                        onClick={() => {
                            if (node.type === 'ACTIVITY') toggleSelection(node.id);
                            else hasChildren && toggleExpand(node.key);
                        }}
                    >
                        <span className="text-gray-400 text-xs mr-2">[{node.code}]</span>
                        {node.name}
                        {node.date && (
    ...
    return (
        <div className="border rounded-md shadow-sm bg-white">
            ...
            <div className="p-2 border-b bg-gray-50 flex flex-col gap-2">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full pl-8 pr-2 py-1.5 text-sm border rounded bg-white focus:outline-blue-500"
                        value={searchTerm}
            ...
            {/* Footer Summary */}
            <div className="px-2 py-1 bg-gray-50 border-t text-xs text-gray-500 text-right">
