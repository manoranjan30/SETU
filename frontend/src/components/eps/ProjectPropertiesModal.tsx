...
import { X, Save, Calendar, User, AlignLeft, Globe, DollarSign, PenTool, Activity } from 'lucide-react';
...
const propertyGroups = [
    {
        ...
        fields: [
            { key: "projectCode", label: "Project Code", type: "string", required: true },
            { key: "projectName", label: "Project Name", type: "string", required: true },
            { key: "projectType", label: "Project Type", type: "enum", options: ["Residential", "Commercial", "Infrastructure", "Mixed"], required: true },
            { key: "projectCategory", label: "Project Category", type: "string" },
            { key: "projectStatus", label: "Project Status", type: "enum", options: ["Planned", "Active", "On-Hold", "Closed"] },
            { key: "projectVersion", label: "Project Version", type: "string" },
            { key: "description", label: "Description", type: "text" },
            { key: "reraRegistrationNumber", label: "RERA Registration Number", type: "string" },
            { key: "reraRegistrationEndDate", label: "RERA Registration End Date", type: "date" }
        ]
    },
    {
    ...
    },
    {
        ...
        fields: [
            ...
            { key: "landOwnershipType", label: "Land Ownership Type", type: "enum", options: ["Owned", "Lease", "JV"] },
            { key: "zoningClassification", label: "Zoning Classification", type: "string" }
        ]
    },
    {
        ...
        fields: [
            ...
            { key: "shiftPattern", label: "Shift Pattern", type: "enum", options: ["Single", "Double", "Triple"] },
    ...
    },
    {
        ...
        fields: [
            ...
            { key: "estimatedProjectCost", label: "Estimated Project Cost", type: "number" },
            { key: "approvedBudget", label: "Approved Budget", type: "number" },
            { key: "fundingType", label: "Funding Type", type: "enum", options: ["Self", "Bank", "JV"] },
            { key: "revenueModel", label: "Revenue Model", type: "enum", options: ["Sale", "Lease", "BOT"] },
    ...
    },
    {
        ...
        fields: [
            { key: "constructionTechnology", label: "Construction Technology", type: "enum", options: ["Conventional", "Aluminium Formwork", "Precast", "Hybrid"] },
            { key: "structuralSystem", label: "Structural System", type: "string" },
            { key: "numberOfBuildings", label: "Number of Buildings", type: "number" },
            { key: "typicalFloorCount", label: "Typical Floor Count", type: "number" },
    ...
    },
    {
        ...
        fields: [
            { key: "lifecycleStage", label: "Project Lifecycle Stage", type: "enum", options: ["Concept", "Design", "Execution", "Handover", "Closeout"] },
            { key: "changeReason", label: "Change Reason", type: "text" }
        ]
    }
];

const ProjectPropertiesModal: React.FC<Props> = ({ isOpen, onClose, nodeId, nodeName }) => {
    ...
    return (
        <div className="fixed inset-0 bg-gray-900 bg-opacity-70 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg shadow-2xl w-[900px] h-[85vh] flex flex-col overflow-hidden">

                {/* Header */}
                <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
                    <div>
                        <h2 className="text-xl font-bold text-gray-800">Project Properties</h2>
                        <p className="text-sm text-gray-500">Managing properties for: <span className="font-semibold text-blue-600">{nodeName}</span></p>
                    </div>
                    <button onClick={onClose} className="p-2 hover:bg-gray-200 rounded-full text-gray-500 hover:text-gray-700">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="flex flex-1 overflow-hidden">
                  ...
                  ${activeGroup === grp.group ? 'bg-white text-blue-600 border-l-4 border-blue-600 shadow-sm' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900 border-l-4 border-transparent'}
                `}
                            >
                    ...
                    <div className="flex-1 p-8 overflow-y-auto bg-white">
                        {loading ? (
                            ...
                            <div className="grid grid-cols-2 gap-x-8 gap-y-6">
                                {propertyGroups.find(g => g.group === activeGroup)?.fields.map((field) => (
                                    <div key={field.key} className={field.type === 'text' ? 'col-span-2' : ''}>
                                        ...
                                        {field.type === 'string' || field.type === 'number' ? (
                                            <input
                                                ...
                                                className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-shadow"
                                            ...
                                            <select
                                                className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none cursor-pointer bg-white"
                                                value={formData[field.key] || ''}
                                                onChange={(e) => handleChange(field.key, e.target.value)}
                                            >
                ...
                <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end gap-3">
                    <button onClick={onClose} className="px-5 py-2 text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 font-medium transition-colors">
                        Cancel
                    </button>
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="px-5 py-2 text-white bg-blue-600 rounded-md hover:bg-blue-700 font-medium shadow-sm flex items-center transition-colors disabled:opacity-50"
                    >
