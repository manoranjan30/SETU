...
const WorkDocTemplateEditor: React.FC<Props> = ({
  isOpen,
  onClose,
  onSuccess,
  template,
  ...
  const [isCoordinateDesignerOpen, setIsCoordinateDesignerOpen] =
    useState(false);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-h-[85vh] overflow-auto pr-2">
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            <h4 className="text-xs font-black text-text-disabled uppercase tracking-widest border-b border-slate-100 pb-2">
              Basic Information
            </h4>
            <div className="space-y-2">
              ...
              <input
                className="w-full p-3 bg-surface-base border border-border-default rounded-xl focus:outline-blue-500 font-bold"
            ...
            <div className="space-y-2">
              ...
              <textarea
                className="w-full p-3 bg-surface-base border border-border-default rounded-xl focus:outline-blue-500 text-sm"
          ...
          <div className="space-y-4">
            <h4 className="text-xs font-black text-text-disabled uppercase tracking-widest border-b border-slate-100 pb-2 flex items-center gap-2">
              Extraction Regex Rules{" "}
              <Info className="w-3 h-3 cursor-help text-blue-400" />
            </h4>
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConfig({ ...config, vendorRegex: e.target.value })
                  }
                />
                <p className="text-[10px] text-text-disabled">
                  Example: Vendor\s*[:#]?\s*(\d+)
                </p>
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold text-text-secondary">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConfig({ ...config, woNumberRegex: e.target.value })
                  }
                />
              </div>
            </div>
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="space-y-4 bg-surface-base p-4 rounded-2xl border border-border-default">
              <div className="space-y-2">
                <label className="text-xs font-bold text-text-secondary">
                  Row Regex (Capturing Groups)
                </label>
                <input
                  className="w-full p-3 bg-surface-card border border-border-default rounded-xl focus:outline-blue-500 font-mono text-xs"
                  ...
                  onChange={(e) =>
                    setConfig({
                      ...
                      tableConfig: {
                        ...config.tableConfig,
                        rowRegex: e.target.value,
                      },
                    })
                  }
                />
                <p className="text-[10px] text-text-disabled">
                  Must use (parentheses) for each column you want to capture.
                </p>
              </div>

              <div className="grid grid-cols-2 gap-4">
                {config.tableConfig?.columnMapping &&
                  Object.entries(config.tableConfig.columnMapping).map(
                    ([field, groupIndex]) => (
                      <div key={field} className="flex flex-col gap-1">
                        <label className="text-[10px] uppercase font-black text-text-muted">
                          {field.replace(/([A-Z])/g, " $1")}
                        </label>
                        <input
                          type="number"
                          className="p-2 bg-surface-card border border-border-default rounded-lg text-xs"
          ...
          <div className="pt-6">
            <button
              onClick={handleSave}
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 px-8 py-4 bg-primary text-white font-black rounded-2xl hover:bg-primary-dark shadow-xl shadow-blue-100 transition-all active:scale-95 disabled:bg-slate-300"
            >
              {loading ? (
        ...
        <div className="bg-slate-900 rounded-3xl p-6 flex flex-col h-full border border-slate-800 shadow-2xl overflow-hidden">
          <div className="flex justify-between items-center border-b border-slate-800 pb-4 mb-6">
            <h4 className="text-xs font-black text-text-muted uppercase tracking-widest flex items-center gap-2">
              <Play className="w-4 h-4 text-success fill-current" /> Live
              Analysis Tester
            </h4>
            <div className="flex bg-slate-800 p-1 rounded-lg">
              <button
                onClick={() => setShowRaw(false)}
                className={`px-3 py-1 text-[10px] font-black rounded-md transition-all ${!showRaw ? "bg-primary text-white shadow-lg" : "text-text-disabled hover:text-slate-200"}`}
              >
                EXTRACTED
              </button>
              <button
          ...
          <div className="flex-1 space-y-6 flex flex-col">
            <div
              className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all cursor-pointer ${testFile ? "border-success/50 bg-success/5" : "border-slate-800 hover:border-primary/50 hover:bg-primary/5"}`}
            ...
            >
              ...
              />
              <FileText
                className={`w-8 h-8 mx-auto mb-2 ${testFile ? "text-success" : "text-text-secondary"}`}
              />
              <p className="text-xs font-bold text-text-disabled">
                {testFile ? testFile.name : "Upload sample PDF to test rules"}
              </p>
            </div>
            ...
            <div className="flex-1 bg-surface-overlay rounded-2xl border border-slate-800 p-4 font-mono text-[10px] overflow-auto text-success/80 custom-scrollbar">
              {testResult ? (
                showRaw ? (
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div className="p-2 border border-slate-800 rounded">
                        <p className="text-text-muted uppercase font-black text-[9px]">
                          Vendor
                        </p>
                        <p className="text-white">
                          {testResult.vendor?.name || "N/A"} (
                          {testResult.vendor?.code})
                        </p>
                      </div>
                      <div className="p-2 border border-slate-800 rounded">
                        <p className="text-text-muted uppercase font-black text-[9px]">
                        ...
                        <p className="text-white">
                          {testResult.header?.woNumber || "N/A"}
                        </p>
                      </div>
                    </div>
                    <div>
                      <p className="text-blue-400 font-black mb-2">
                        Rows Extracted: {testResult.items?.length || 0}
                      </p>
                      <div className="space-y-2">
                        {testResult.items
                          ?.slice(0, 5)
                          .map((item: any, i: number) => (
                            <div
                              key={i}
                              className="p-2 bg-slate-800/50 rounded border-l-2 border-success"
                            >
                              <p className="text-white font-bold">
                                {item.code}
                              </p>
                              <p className="text-text-disabled truncate">
                                {item.description}
                              </p>
                        ...
                        {(testResult.items?.length || 0) > 5 && (
                          <p className="text-text-muted italic">
                            ... and {(testResult.items?.length || 0) - 5} more
                          </p>
                        )}
                      </div>
                    </div>
                ...
                <p className="text-slate-600">
                  No test results yet. Upload a PDF and click 'Run Test
                  Analysis'.
                </p>
              )}
            </div>
