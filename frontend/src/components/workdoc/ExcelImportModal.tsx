...
import { WorkDocService, type ColumnMapping, type ConfirmWorkOrderData } from '../../services/work-doc.service';
import { toast } from 'react-hot-toast';
import {
    FileSpreadsheet, Loader2, ChevronRight, ChevronDown,
    ArrowRight, ArrowLeft, AlertCircle, Indent, Outdent, Save
} from 'lucide-react';
...
const ExcelImportModal: React.FC<Props> = ({ isOpen, onClose, onSuccess, vendorId }) => {
    ...
    const handleFileSelect = async (selectedFile: File) => {
        ...
        try {
            const data = await WorkDocService.previewExcelFn(Number(projectId), selectedFile);
    ...
    const handleImport = async () => {
        ...
        const missingRequired = COLUMN_OPTIONS
            .filter(opt => opt.required && columnMapping[opt.key as keyof ColumnMapping] === undefined)
            .map(opt => opt.label);
        ...
        try {
            const data = await WorkDocService.importExcelFn(Number(projectId), file, columnMapping, headerRow);

            // Add temporary IDs and expansion state
            const itemsWithIds: ImportedItem[] = data.items.map((item, idx: number) => ({
                ...item,
                id: `item-${idx}`,
                isExpanded: true,
            }));
    ...
    const outdentItem = (index: number) => {
        ...
        if (item.level > 0) {
            ...
            const maxSerial = Math.max(...parentItems.map(i => parseInt(i.serialNumber) || 0));
    ...
    const recalculateAmounts = (itemList: ImportedItem[]) => {
        ...
        for (const item of newItems) {
            if (item.isParent) {
                const children = newItems.filter(i => i.parentSerialNumber === item.serialNumber);
                item.calculatedAmount = children.reduce((sum, c) => sum + c.calculatedAmount, 0);
            }
        }
    ...
    // Update item field
    const updateItem = <K extends keyof ImportedItem>(index: number, field: K, value: ImportedItem[K]) => {
        const newItems = [...items];
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (window.confirm('Import successful. Do you want to review unmapped items in the Pending Board?')) {
        ...
        return (
            <Modal isOpen={isOpen} onClose={onClose} title="Import from Excel/CSV">
                <div className="space-y-6 py-4">
                    <div
                        className={`border-2 border-dashed rounded-2xl p-12 text-center transition-all cursor-pointer ${file ? 'border-green-500 bg-green-50' : 'border-gray-300 hover:border-blue-400 hover:bg-gray-50'
                            }`}
                        onClick={() => document.getElementById('excel-file')?.click()}
                    >
                        <div className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${file ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-400'
                            }`}>
                        ...
                        {loading ? (
                            <div className="flex items-center justify-center gap-2">
                                ...
                                <span className="text-blue-600 font-medium">Reading file...</span>
                            </div>
                        ) : file ? (
                            <div>
                    ...
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                        ...
                        <ul className="text-sm text-amber-700 mt-2 space-y-1">
                            <li>• <strong>Serial No.</strong> - Hierarchy indicator (10, 10.1, 10.2, 20, etc.)</li>
                            <li>• <strong>SAP Item Number</strong> - Material/Service code</li>
                            <li>• <strong>Short/Detail Description</strong> - Item descriptions</li>
                            <li>• <strong>UOM, Quantity, Rate, Amount</strong> - Measurements</li>
                        </ul>
                    </div>
                </div>
            </Modal>
            ...
            <Modal isOpen={isOpen} onClose={onClose} title="Map Columns">
                <div className="space-y-6 max-h-[80vh] overflow-auto">
                    ...
                    <div>
                        ...
                        <div className="border rounded-xl overflow-auto max-h-48">
                            <table className="w-full text-xs">
                                ...
                                <tbody>
                                    {previewData.previewRows.slice(headerRow, headerRow + 5).map((row, idx) => (
                    ...
                    <div className="flex justify-between pt-4 border-t">
                        <button onClick={() => setStep('upload')} className="flex items-center gap-2 text-slate-600 hover:text-slate-800">
            ...
            <div className="space-y-4 max-h-[80vh] flex flex-col">
                ...
                <div className="flex gap-4 text-sm">
                    <div className="bg-blue-50 px-4 py-2 rounded-lg">
                        <span className="text-blue-600 font-bold">{items.filter(i => i.isParent).length}</span>
                        <span className="text-blue-500 ml-1">Parent Items</span>
                    </div>
                    <div className="bg-green-50 px-4 py-2 rounded-lg">
                        <span className="text-green-600 font-bold">{items.filter(i => !i.isParent).length}</span>
                        <span className="text-green-500 ml-1">Child Items</span>
                    </div>
                    <div className="bg-slate-50 px-4 py-2 rounded-lg">
                        <span className="text-slate-700 font-bold">
                            ₹{items.filter(i => i.level === 0).reduce((s, i) => s + i.calculatedAmount, 0).toLocaleString()}
                        </span>
                    ...
                    <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {visibleItems.map((item) => {
                                ...
                                const hasChildren = items.some(i => i.parentSerialNumber === item.serialNumber);
                                    ...
                                    >
                                        ...
                                        <td className="px-3 py-2">
                                            <input
                                                ...
                                                onChange={e => updateItem(originalIdx, 'materialCode', e.target.value)}
                                            />
                                        </td>
                                        ...
                                        <td className="px-3 py-2">
                                            <input
                                                ...
                                                onChange={e => updateItem(originalIdx, 'shortText', e.target.value)}
                                            />
                                        </td>
                                        ...
                                        <td className="px-3 py-2">
                                            <input
                                                ...
                                                value={item.uom}
                                                onChange={e => updateItem(originalIdx, 'uom', e.target.value)}
                                            />
                                        </td>
                                        ...
                                        <td className="px-3 py-2 text-right">
                                            {!item.isParent ? (
                                                <input
                                                    ...
                                                    value={item.quantity}
                                                    onChange={e => updateItem(originalIdx, 'quantity', Number(e.target.value))}
                                                />
                                            ) : (
                                        ...
                                        <td className="px-3 py-2 text-right">
                                            {!item.isParent ? (
                                                <input
                                                    ...
                                                    value={item.rate}
                                                    onChange={e => updateItem(originalIdx, 'rate', Number(e.target.value))}
                                                />
                                            ) : (
                                                <span className="text-slate-400">-</span>
                                            )}
                ...
                {/* Actions */}
                <div className="flex justify-between pt-4 border-t">
                    <button onClick={() => setStep('mapping')} className="flex items-center gap-2 text-slate-600 hover:text-slate-800">
