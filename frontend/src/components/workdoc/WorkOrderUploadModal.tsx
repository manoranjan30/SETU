...
const WorkOrderUploadModal: React.FC<Props> = ({
  isOpen,
  onClose,
  onSuccess,
}) => {
  ...
  const [step, setStep] = useState<
    "vendor" | "pdf_upload" | "excel_upload" | "excel_map" | "review"
  >("vendor");
  ...
  const handleExcelImport = async () => {
    ...
    const missingRequired = COLUMN_OPTIONS.filter(
      (opt) => opt.required && columnMapping[opt.key] === undefined,
    ).map((opt) => opt.label);
    ...
    try {
      ...
      const response = await api.post(
        `/workdoc/${projectId}/import-excel`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        },
  ...
  const indentItem = (index: number) => {
    ...
    for (let i = index - 1; i >= 0; i--) {
      ...
      if (
        prevItem &&
        (prevItem.level === 0 || (prevItem.level || 0) < (item?.level || 0))
      ) {
  ...
  const outdentItem = (index: number) => {
    ...
    if ((item.level || 0) > 0) {
      ...
      const maxSerial = Math.max(
        ...parentItems.map((i) => parseInt(i.serialNumber || "0") || 0),
      );
    ...
    for (let i = newItems.length - 1; i >= 0; i--) {
      ...
      if (item.isParent) {
        const children = newItems.filter(
          (c) => c.parentSerialNumber === item.serialNumber,
        );
  ...
  if (step === "vendor") {
    return (
      <Modal
        isOpen={isOpen}
        onClose={handleClose}
        title="Step 1: Select or Create Vendor"
      >
        <div className="space-y-6 py-4">
          ...
          {vendorMode === "new" && (
            <div className="text-center p-8 bg-success-muted rounded-xl border border-green-200">
              ...
              <p className="text-sm text-success mb-4">
                You'll need to register the vendor before importing their Work
                Order.
              </p>
              <button
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border-default">
            ...
            <button
              ...
              className="px-6 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark disabled:bg-gray-300 disabled:cursor-not-allowed transition-all shadow-lg"
            >
  ...
  if (step === "pdf_upload") {
    return (
      <Modal
        isOpen={isOpen}
        onClose={handleClose}
        title="Step 2: Upload Work Order PDF (Header Info)"
      >
        <div className="space-y-6 py-4">
          <div className="bg-primary-muted border border-blue-200 rounded-xl p-4 flex gap-3 items-start">
            <div className="p-2 bg-surface-card rounded-lg shadow-sm">
              <FileText className="w-6 h-6 text-primary" />
            </div>
            <div>
              <h4 className="font-bold text-blue-900">
                Uploading for: {selectedVendor?.name}
              </h4>
              <p className="text-xs text-blue-700 mt-1">
                We will extract Work Order Number, Date, and Project details
                from this PDF.
              </p>
            </div>
          </div>
            ...
            {file ? (
              ...
              <div>
                <p className="text-text-primary font-bold">
                  Click to Upload Work Order PDF
                </p>
          ...
          {templates.length > 0 && (
            <div>
              ...
              <select
                className="w-full p-2 border rounded-lg bg-surface-card"
          ...
          <div className="flex justify-between gap-3 pt-4 border-t border-border-default">
            ...
            <button
              ...
              className="px-8 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primary-dark disabled:bg-gray-300 disabled:cursor-not-allowed transition-all shadow-lg flex items-center gap-2"
            >
  ...
  if (step === "excel_upload") {
    return (
      <Modal
        isOpen={isOpen}
        onClose={handleClose}
        title="Step 3: Upload Excel (Detail Items)"
      >
        <div className="space-y-6 py-4">
          <div className="bg-success-muted border border-green-200 rounded-xl p-4 flex gap-3 items-start">
            <div className="p-2 bg-surface-card rounded-lg shadow-sm">
            ...
            <div>
              ...
              <p className="text-xs text-green-700 mt-1">
                Now please upload the Excel/CSV file containing the detailed
                Work Order Items (Quantities, Rates, Amounts) to complete the
                import.
              </p>
            </div>
          </div>
          ...
          <div
            className={`border-2 border-dashed rounded-2xl p-12 text-center transition-all cursor-pointer ${
              excelFile
                ...
                : "border-border-strong hover:border-green-400 hover:bg-surface-base"
            }`}
  ...
  if (step === "excel_map" && excelPreview) {
    ...
    return (
      <Modal
        isOpen={isOpen}
        onClose={handleClose}
        title="Step 4: Map Excel Columns"
      >
  ...
  if (step === "review" && parsedData) {
    return (
      <Modal
        isOpen={isOpen}
        onClose={handleClose}
        title="Step 5: Final Review & Confirm"
      >
        <div className="space-y-6 max-h-[85vh] flex flex-col">
          <div className="bg-primary-muted border border-blue-100 p-4 rounded-xl flex gap-4 items-start">
            <CheckCircle className="text-primary shrink-0 mt-1" />
            <div>
              <h4 className="font-bold text-blue-900">Import Ready!</h4>
              <p className="text-blue-700 text-sm">
                Merged Header (from PDF) and {excelItems.length} Items (from
                Excel). Please verify before saving.
              </p>
            </div>
          </div>

          {/* Work Order Details (Header) */}
          <div className="bg-surface-base p-4 rounded-xl border border-border-default shrink-0">
            <h3 className="text-xs font-black text-text-muted uppercase mb-3 flex items-center gap-2">
              <FileText className="w-4 h-4" /> Work Order Header
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
              <div>
                <label className="text-[10px] text-text-disabled uppercase">
                  Order No.
                </label>
                <input
                  className="w-full bg-surface-card border rounded px-2 py-1 font-bold text-primary"
              ...
              <div>
                ...
                <input
                  type="date"
                  className="w-full bg-surface-card border rounded px-2 py-1"
              ...
              <tbody className="divide-y divide-slate-100">
                {excelItems
                  .filter(
                    (item) =>
                      item.level === 0 ||
                      !collapsedParents.has(item.parentSerialNumber || ""),
                  )
                  .map((item) => {
                    // Need to find original index for updates
                    const originalIdx = excelItems.findIndex(
                      (i) => i.id === item.id,
                    );
                    const hasChildren = excelItems.some(
                      (i) => i.parentSerialNumber === item.serialNumber,
                    );
                    const isCollapsed = collapsedParents.has(
                      item.serialNumber || "",
                    );
                    ...
                    return (
                      <tr
                        key={item.id}
                        className={`hover:bg-primary-muted/50 transition-colors ${item.isParent ? "bg-surface-base font-semibold" : ""}`}
                      >
                        {/* Expand/Collapse */}
                        <td className="px-3 py-2">
                          {hasChildren && (
                            <button
                              onClick={() =>
                                toggleCollapse(item.serialNumber || "")
                              }
