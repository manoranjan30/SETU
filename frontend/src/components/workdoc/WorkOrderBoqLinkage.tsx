...
const WorkOrderBoqLinkage: React.FC<Props> = ({ workOrder, onRefresh }) => {
    ...
    const [selectedWoItem, setSelectedWoItem] = useState<WorkOrderItem | null>(null);
    const [tempMappings, setTempMappings] = useState<Record<number, { boqItemId?: number; boqSubItemId?: number; factor: number }[]>>({});
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const res = await api.get(`/workdoc/${workOrder.id ? workOrder.projectId : 0}/linkage-data?woId=${workOrder.id}`);
    ...
    const handleFactorChange = (woItemId: number, index: number, factor: number) => {
        const current = [...(tempMappings[woItemId] || [])];
    ...
    const saveMapping = async (woItemId: number) => {
        setSaving(true);
        try {
            await api.post(`/workdoc/items/${woItemId}/map`, tempMappings[woItemId] || []);
    ...
    return (
        <div className="flex h-full min-h-0 bg-white shadow-inner rounded-xl overflow-hidden m-4 border border-slate-200">
            {/* Left: WO Items */}
            <div className="w-1/2 border-r border-slate-200 flex flex-col bg-slate-50/30">
                <div className="p-4 border-b bg-white flex justify-between items-center">
                    <div>
                        <h3 className="font-black text-slate-800 tracking-tight">Work Order Items</h3>
                        <p className="text-[10px] text-slate-400 font-bold uppercase">Select a line to link with BOQ</p>
                    </div>
                </div>
                <div className="flex-1 overflow-auto p-4 space-y-3">
                    {woItems.map(item => (
                        <div
                            ...
                            className={`p-4 rounded-2xl border-2 transition-all cursor-pointer ${selectedWoItem?.id === item.id
                                ? 'border-blue-500 bg-blue-50 shadow-lg shadow-blue-100'
                                : 'border-slate-100 bg-white hover:border-blue-200'
                                }`}
                        >
                            <div className="flex justify-between items-start mb-2">
                                <span className="text-[10px] font-mono text-slate-400">#{item.serialNumber}</span>
                                <div className="flex gap-1">
                                    {(tempMappings[item.id]?.length || 0) > 0 && (
                                        <span className="px-2 py-0.5 rounded-full bg-green-500 text-white text-[10px] font-black italic">
                            ...
                            <div className="mt-2 flex items-center justify-between">
                                ...
                                {selectedWoItem?.id === item.id && (
                                    <button
                                        ...
                                        disabled={saving}
                                        className="flex items-center gap-1.5 px-3 py-1 bg-blue-600 text-white text-[10px] font-black rounded-lg hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200"
                                    >
                            ...
                            {selectedWoItem?.id === item.id && (tempMappings[item.id]?.length || 0) > 0 && (
                                <div className="mt-4 pt-3 border-t border-blue-100 space-y-2">
                                    <p className="text-xs font-black text-blue-800 uppercase tracking-widest">Mappings</p>
                                    {tempMappings[item.id].map((m, idx) => {
                                        const boqItem = boqItems.find(bi => bi.id === m.boqItemId);
                                        const subItem = boqItem?.subItems?.find((si: any) => si.id === m.boqSubItemId);
                                        return (
                                            <div key={idx} className="bg-white/50 p-2 rounded-xl border border-blue-100 flex items-center gap-3">
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-[10px] font-bold text-slate-400 italic truncate">
                                                        {boqItem?.boqCode} {subItem ? `> ${subItem.description}` : boqItem?.description}
                                                    </p>
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <span className="text-[10px] font-black text-slate-600 uppercase">Factor:</span>
                                                        <input
                                                            type="number"
                                                            className="w-16 text-[10px] border border-blue-200 rounded px-1 focus:ring-1 focus:ring-blue-500"
                                                            value={m.factor}
                                                            onChange={(e) => handleFactorChange(item.id, idx, parseFloat(e.target.value) || 0)}
                                                        />
                                                    </div>
                                                </div>
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); handleUnlink(item.id, idx); }}
            ...
            <div className="w-1/2 flex flex-col bg-white">
                <div className="p-4 border-b flex justify-between items-center">
                    <div>
                        <h3 className="font-black text-slate-800 tracking-tight">Available BOQ Items</h3>
                        <p className="text-[10px] text-slate-400 font-bold uppercase">Check a node to link with selection</p>
                    </div>
