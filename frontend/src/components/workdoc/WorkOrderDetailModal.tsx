...
const ALL_COLUMNS: ColumnDef[] = [
  ...
  },
  {
    ...
    defaultVisible: true,
    render: (item) => (
      <span className="truncate text-slate-800" title={item.description}>
        {item.description}
      </span>
    ),
  },
  {
    ...
    render: (item) => (
      <span className="truncate text-xs text-text-muted" title={item.longText}>
        {item.longText || "-"}
      </span>
    ),
  },
  {
    ...
    defaultVisible: true,
    editable: true,
    render: (item, isParent, _, onEdit) =>
      isParent ? (
        "-"
      ) : (
        <input
          type="number"
          className="w-full text-right bg-transparent border-0 focus:ring-1 focus:ring-primary rounded px-1 font-bold"
          defaultValue={item.allocatedQty}
          onBlur={(e) =>
            onEdit?.("allocatedQty", parseFloat(e.target.value) || 0)
          }
  ...
  },
  {
    ...
    defaultVisible: true,
    render: (item) => (
      <span className="text-xs text-text-muted">{item.uom}</span>
    ),
  },
  {
    key: "rate",
    ...
    render: (item, isParent, _, onEdit) =>
      isParent ? (
        ...
        <input
          type="number"
          className="w-full text-right bg-transparent border-0 focus:ring-1 focus:ring-primary rounded px-1 text-xs"
    ...
    render: (item) => (
      <div className="flex flex-col items-end">
        <span className="font-bold text-green-700">
          {Number(item.executedQuantity || 0).toFixed(3)}
        </span>
        <span className="text-[10px] text-text-disabled">
          of {item.allocatedQty} {item.uom}
        </span>
      </div>
    ),
  },
  ...
  },
  {
    ...
    defaultVisible: false,
    render: (item) => (
      <span className="text-xs text-text-disabled">{item.level ?? 0}</span>
    ),
  },
  {
    key: "id",
    ...
    defaultVisible: false,
    render: (item) => (
      <span className="text-xs text-text-disabled font-mono">{item.id}</span>
    ),
  },
];
...
const TableRow = (
  ...
  const hasChildren =
    !!item.serialNumber &&
    sortedItems.some(
      (i: WorkOrderItem) => i.parentSerialNumber === item.serialNumber,
    );
  ...
  return (
    <div
      style={style}
      className={clsx(
        "flex items-center border-b border-border-default hover:bg-primary-muted/50 transition-colors text-sm",
        isParent
      ...
      {ALL_COLUMNS.filter((col) => visibleColumns[col.key]).map((col) => {
        ...
        if (col.key === "toggle") {
          return (
            <div
              key={col.key}
              style={{ width }}
              className="flex justify-center flex-shrink-0 border-r border-border-default"
            >
              {hasChildren && (
                <button
        ...
        if (col.key === "serial") {
          return (
            <div
              ...
              className="flex-shrink-0 truncate border-r border-border-default px-2 flex items-center h-full"
            >
        ...
        return (
          <div
            ...
            style={{ width }}
            className={clsx(
              "flex-shrink-0 px-2 border-r border-border-default flex items-center h-full",
              col.align === "right" && "justify-end",
              col.align === "center" && "justify-center",
              col.align === "left" && "justify-start",
              col.key === "calcAmount" && "bg-primary-muted/30",
            )}
          >
            {col.render(item, isParent, calculatedAmount, handleEdit)}
          </div>
...
const WorkOrderDetailModal: React.FC<Props> = ({
  ...
  const collapseAll = useCallback(() => {
    const allParents = sortedItems
      .filter(
        (item) =>
          item.isParent ||
          sortedItems.some((i) => i.parentSerialNumber === item.serialNumber),
      )
      .map((item) => item.serialNumber || "");
  ...
  const handleEditItem = useCallback(
    async (itemId: number, field: string, value: number) => {
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent, item: WorkOrderItem) => {
      e.preventDefault();
  ...
  const totalWidth = useMemo(() => {
    return ALL_COLUMNS.filter((col) => visibleColumns[col.key]).reduce(
      (acc, col) => acc + (colWidths[col.key] || col.defaultWidth),
      0,
    );
  ...
  return (
    ...
    >
      <div className="h-full min-h-0 flex flex-col p-0 bg-surface-base/50">
        ...
        <div className="bg-surface-card border-b border-border-default px-4 py-3 md:px-5 grid grid-cols-1 gap-4 md:grid-cols-5 md:gap-5 shadow-sm z-10">
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              <div className="w-7 h-7 rounded-full bg-primary-muted text-primary flex items-center justify-center font-bold text-xs border border-blue-100">
          ...
          <div>
                ...
                className={clsx(
                  ...
                  workOrder.status === "ACTIVE"
                    ...
                    : workOrder.status === "DRAFT"
                      ...
                      : "bg-slate-100 text-slate-600 border border-border-default",
                )}
              >
                {workOrder.status}
              </span>
            ...
            <p className="text-[10px] text-text-muted">
              {new Date(workOrder.woDate).toLocaleDateString()}
            </p>
          </div>
          <div className="col-span-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-8 py-2 border border-border-default rounded-lg text-sm focus:ring-2 focus:ring-primary focus:border-primary"
              />
              {searchTerm && (
                <button
          ...
          <button
            ...
            className={clsx(
              ...
              activeTab === "items"
                ...
                : "border-transparent text-text-disabled hover:text-slate-600",
            )}
          >
        ...
        {activeTab === "items" && (
          <>
            ...
            <div className="bg-surface-card border-b border-border-default px-3 py-2 md:px-4 flex flex-wrap items-center gap-2 z-10">
                ...
                {allLevels.map((level) => (
                  <div key={level} className="flex items-center">
                    <button
                      ...
                      className="px-2 py-1 text-xs bg-slate-100 hover:bg-info-muted rounded-l border border-slate-300 text-slate-600"
                      title={`Expand all Level ${level}`}
                    >
                      <ChevronDown size={12} />
                    </button>
                    <span className="px-2 py-1 text-xs bg-surface-base border-t border-b border-slate-300 text-text-secondary font-medium">
                      {level}
                    </span>
                    <button
              ...
              <button
                onClick={expandAll}
                className="px-3 py-1.5 text-xs bg-primary-muted hover:bg-info-muted rounded border border-blue-200 text-blue-700 flex items-center gap-1"
              >
                <ChevronsUpDown size={14} /> Expand All
              </button>
              <button
                onClick={collapseAll}
                className="px-3 py-1.5 text-xs bg-orange-50 hover:bg-orange-100 rounded border border-orange-200 text-orange-700 flex items-center gap-1"
              >
              ...
              <div className="relative">
                <button
                  ...
                  className="px-3 py-1.5 text-xs bg-slate-100 hover:bg-slate-200 rounded border border-slate-300 text-text-secondary flex items-center gap-1"
                >
                  <Settings size={14} /> Columns
                </button>
                {showColMenu && (
                  <div className="absolute right-0 mt-1 w-56 bg-surface-card border border-border-default rounded-lg shadow-xl p-3 z-50 max-h-80 overflow-auto">
                    <div className="font-bold text-text-secondary text-xs uppercase tracking-wider mb-2 pb-2 border-b border-border-default">
                      Column Visibility
                    </div>
                    {ALL_COLUMNS.filter((col) => col.key !== "toggle").map(
                      (col) => (
                        <label
                          key={col.key}
                          className="flex items-center py-1 cursor-pointer hover:bg-surface-base rounded px-1"
                        >
                          <input
                            type="checkbox"
                          ...
                          <span
                            className={clsx(
                              "text-sm",
                              col.alwaysVisible && "text-text-disabled",
                            )}
                          >
                            {col.label}
                          </span>
                    ...
                    (col) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {col.key !== "toggle" && (
                            <div
                              className="absolute right-0 top-0 bottom-0 w-1 cursor-col-resize hover:bg-primary opacity-0 group-hover:opacity-100 transition-opacity z-10"
        ...
        {contextMenu && (
          <div
            className="fixed bg-surface-card border border-border-default rounded-lg shadow-xl py-1 z-[100] min-w-[180px]"
            style={{ left: contextMenu.x, top: contextMenu.y }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="px-3 py-1 text-[10px] text-text-disabled uppercase tracking-wider border-b border-slate-100 mb-1">
              Level {contextMenu.item.level ?? 0}
            </div>
            <button
              onClick={() => {
                expandLevel(contextMenu.item.level ?? 0);
                setContextMenu(null);
              }}
              className="w-full px-3 py-1.5 text-left text-sm hover:bg-primary-muted text-text-secondary flex items-center gap-2"
            >
              <ChevronDown size={14} /> Expand All at This Level
            </button>
            <button
              onClick={() => {
                collapseLevel(contextMenu.item.level ?? 0);
                setContextMenu(null);
              }}
              className="w-full px-3 py-1.5 text-left text-sm hover:bg-orange-50 text-text-secondary flex items-center gap-2"
            >
            ...
            <div className="border-t border-slate-100 mt-1 pt-1">
              <button
                ...
                className="w-full px-3 py-1.5 text-left text-sm hover:bg-surface-base text-text-secondary flex items-center gap-2"
              >
