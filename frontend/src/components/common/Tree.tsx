...
import { ChevronRight, ChevronDown, Folder, File, CheckCircle, Circle } from 'lucide-react';
...
const TreeNode: React.FC<{
    ...
    renderLabel?: (node: TreeNodeData) => React.ReactNode;
    getItemClassName?: (node: TreeNodeData) => string;
}> = ({ node, level, selectedIds, onToggle, renderLabel, getItemClassName }) => {
    ...
    return (
        <div style={{ paddingLeft: `${level * 12}px` }}>
            <div
                className={`flex items-center gap-2 py-1 px-2 rounded cursor-pointer hover:bg-gray-100 ${isSelected ? 'bg-blue-50' : ''} ${customClass}`}
            ...
            >
                ...
                {/* Checkbox / Selection */}
                <div onClick={handleCheck} className={`cursor-pointer ${isSelected ? 'text-blue-600' : 'text-gray-300'}`}>
                    {isSelected ? <CheckCircle size={16} /> : <Circle size={16} />}
                </div>

                {/* Icon */}
                <div className="text-gray-500">
                    {node.icon || (hasChildren ? <Folder size={16} className="fill-yellow-100 text-yellow-600" /> : <File size={16} />)}
                </div>

                {/* Label */}
                <div className="flex-1 text-sm select-none truncate">
...
export const Tree: React.FC<TreeProps> = ({ data, selectedIds, onSelect, renderLabel, getItemClassName, cascadeSelection = true }) => {

    const handleToggle = (id: string | number, allChildren: (string | number)[]) => {
        ...
        if (cascadeSelection) {
            ...
            if (isSelected) {
                ...
                const toAdd = allChildren.filter(childId => !selectedIds.includes(childId));
