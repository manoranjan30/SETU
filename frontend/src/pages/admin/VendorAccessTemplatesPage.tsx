    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
                <div>
                    <h1 className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-teal-600 to-emerald-600">Vendor Access Templates</h1>
                    <p className="text-sm text-gray-500 mt-1">Manage role ceilings for temporary vendor users</p>
                </div>
                <button
                    onClick={() => {
                        setEditing(null);
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {templates.map(t => (
                            <tr key={t.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <button onClick={() => openEdit(t)} className="text-indigo-600 hover:text-indigo-900 mr-4">Edit</button>
                                    {t.isActive && (
                                        <button onClick={() => handleDeactivate(t.id)} className="text-red-600 hover:text-red-900">Deactivate</button>
                                    )}
                                </td>
                            </tr>
                        ))}
                        {templates.length === 0 && (
                            <tr>
                                <td colSpan={5} className="px-6 py-8 text-center text-gray-500">No templates found</td>
                            </tr>
                        )}
                    </tbody>
                </table>
                    ...
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl flex flex-col max-h-[90vh]">
                        ...
                        <div className="p-6 flex-1 overflow-y-auto">
                            <div className="space-y-4">
                                ...
                                <div className="mt-6">
                                    <label className="block text-sm font-medium text-gray-700 mb-3 border-b pb-2">Allowed Permissions</label>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {Object.entries(TEMP_USER_ASSIGNABLE_PERMISSIONS).map(([module, perms]) => (
                                            <div key={module} className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                                                <h4 className="text-sm font-semibold text-gray-800 mb-3">{module}</h4>
                                                <div className="space-y-2">
                                                    {perms.map((p) => (
                                                        <label key={p.key} className="flex items-start space-x-3">
                                                            <input
                                                                type="checkbox"
                                                                checked={formData.allowedPermissions.includes(p.key)}
                        ...
                        <div className="px-6 py-4 border-t border-gray-100 flex justify-end space-x-3 bg-gray-50 rounded-b-xl">
                            ...
                            <button
                                onClick={handleSave}
                                disabled={!formData.name || formData.allowedPermissions.length === 0}
