  ...
  const handleUpdateActivity = async (
    ...
    try {
      ...
      if (versionId) {
        ...
        const currentActivity = scheduleData?.activities.find(
          (a) => a.id === activityId,
        );
        ...
        if (field === "startDatePlanned" || field === "finishDatePlanned") {
          payload.startDate =
            field === "startDatePlanned"
              ? value
              : currentActivity.startDatePlanned;
          payload.finishDate =
            field === "finishDatePlanned"
              ? value
              : currentActivity.finishDatePlanned;
        }

        await api.patch(
          `/planning/versions/${versionId}/activities/${activityId}?projectId=${projectId}`,
          payload,
        );
      } else {
        // MASTER MODE UPDATE
        await api.patch(`/projects/${projectId}/wbs/activities/${activityId}`, {
          [field]: value,
        });
      }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-6 py-4 border-b border-border-default bg-surface-card flex justify-between items-center">
        ...
        <div className="flex gap-2 items-center">
          {/* Zoom Controls */}
          <div className="bg-surface-raised p-1 rounded-lg flex mr-2">
            <button
              onClick={() => setZoomLevel(Math.max(0.5, zoomLevel - 0.1))}
              className="px-3 py-1 text-text-secondary hover:bg-surface-card rounded"
            >
              -
            </button>
            <span className="px-2 py-1 text-xs text-text-muted min-w-[3rem] text-center">
          ...
          <button
            onClick={async () => {
              if (
                !confirm(
                  "This will recalculate all activity DURATIONS based on their Planned Start and Finish dates using the Project Calendar. This creates proper alignment for CPM. Continue?",
                )
              )
                return;
              try {
                setLoading(true);
                await api.post(
                  `/projects/${projectId}/schedule/repair-durations`,
                );
          ...
          <button
            onClick={async () => {
              if (
                !confirm(
                  "This will Synchronize your PLANNED dates to match the CPM Calculated dates. Your original dates will be saved as MSP Dates. Continue?",
                )
              )
                return;
              try {
          ...
          <button
            ...
            className="flex items-center px-4 py-2 bg-secondary text-white rounded-md hover:bg-secondary-dark font-medium shadow-sm transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center px-4 py-2 bg-surface-card border border-border-strong text-text-secondary rounded-md hover:bg-surface-base font-medium shadow-sm"
          >
      ...
      {/* Content */}
      <div className="flex-1 overflow-hidden bg-surface-base p-4">
        {loading ? (
          <div className="flex items-center justify-center h-full text-text-muted">
            Loading Schedule...
          </div>
        ) : error ? (
          <div className="flex items-center justify-center h-full text-error font-bold bg-surface-card rounded shadow p-10">
            Error Loading Schedule: {error}
          </div>
        ) : !scheduleData ? (
          <div className="flex items-center justify-center h-full text-text-disabled font-medium bg-surface-card rounded shadow p-10">
            No Schedule Data Found. Please Import a Project.
          </div>
        ) : (
          <div className="bg-surface-card rounded-lg shadow h-full overflow-hidden flex flex-col">
            <ScheduleTable
