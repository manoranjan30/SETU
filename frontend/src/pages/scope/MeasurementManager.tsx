...
import { Download, Upload, Plus, Calculator, ArrowLeft, Trash2, Save, Layout, ChevronDown } from 'lucide-react';
import { boqService } from '../../services/boq.service';
import { tableViewService, type TableViewConfig } from '../../services/table-view.service';
...
export const MeasurementManager: React.FC<MeasurementManagerProps> = ({ projectId, boqItem, subItem, onClose, onUpdate, epsNodes }) => {
    ...
    const onMouseMove = (e: MouseEvent) => {
        ...
        setColumns(cols => cols.map(c => c.id === id ? { ...c, width: Math.max(40, startWidth + diff) } : c));
    };
    ...
    const handleAddManual = async () => {
        ...
        try {
            await boqService.addMeasurement({
                ...
                baseCoordinates: newM.baseCoordinates ? JSON.parse(newM.baseCoordinates) : undefined, // Parse text to JSON
                plineAllLengths: newM.plineAllLengths ? JSON.parse(newM.plineAllLengths) : undefined,
                qty: newM.qty
            });
            alert('Measurement added!');
            setNewM({
                epsNodeId: '', elementName: '', elementCategory: '', elementType: '', linkingElement: '', grid: '', uom: '',
                bottomLevel: '', topLevel: '',
                length: 0, breadth: 0, depth: 0, height: 0, perimeter: 0, baseArea: 0,
                baseCoordinates: '', plineAllLengths: '',
                qty: 0
            });
    ...
    const toggleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            const allIds = new Set<number>(measurements.map((m: any) => Number(m.id)));
    ...
    const handleBulkDelete = async () => {
        ...
        if (!confirm(`Are you sure you want to delete ${selectedIds.size} measurements? This action cannot be undone.`)) return;
    ...
    return (
        <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center z-50 overflow-hidden">
            ...
            <div className="bg-white w-full h-full md:m-4 md:w-[98%] md:h-[95%] md:rounded-lg shadow-2xl flex flex-col">

                {/* Header */}
                <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50 rounded-t-lg">
                    <div>
                        <div className="flex items-center gap-2">
                            <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded text-xs font-bold uppercase tracking-wider">Measurement Sheet</span>
                            <h2 className="text-xl font-bold text-gray-800">{boqItem.boqCode} - {subItem?.description || boqItem.description}</h2>
                        </div>
                        <p className="text-sm text-gray-500 mt-1">Manage detailed measurements and manual entries</p>
                    </div>
                    <div className="flex items-center gap-2">
                        <button onClick={onUpdate} className="text-blue-600 hover:text-blue-800 text-sm font-medium px-3">
                            Refresh
                        </button>
                ...
                <div className="flex-1 flex flex-col p-4 overflow-hidden gap-4">
                    ...
                    <div className="flex justify-between items-center">
                        <div className="flex gap-2 items-center">
                            ...
                            <div className="relative">
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-300 rounded text-gray-700 hover:bg-gray-50 text-sm shadow-sm"
                                    title="Table Settings"
                                >
                                    <Layout className="w-4 h-4 text-blue-600" />
                                    <span className="font-medium text-blue-900">{currentViewName}</span>
                                    <ChevronDown className="w-3 h-3 text-gray-400" />
                                </button>

                                {isViewMenuOpen && (
                                    <div className="absolute top-10 left-0 w-64 bg-white shadow-xl rounded-lg border border-gray-200 z-50 p-3 max-h-[80vh] overflow-y-auto">
                                        <h4 className="text-xs font-bold text-gray-500 uppercase mb-2">Saved Views</h4>
                                        <div className="space-y-1 mb-3">
                                            <button onClick={handleResetView} className={`w-full text-left px-2 py-1 rounded text-sm ${currentViewName === 'Default' ? 'bg-blue-50 text-blue-700 font-bold' : 'hover:bg-gray-50'}`}>Default</button>
                                            {views.map(v => (
                                                <button key={v.id} onClick={() => handleApplyView(v)} className={`w-full text-left px-2 py-1 rounded text-sm flex justify-between ${currentViewName === v.viewName ? 'bg-blue-50 text-blue-700 font-bold' : 'hover:bg-gray-50'}`}>
                                                    {v.viewName}
                                                    <span className="text-xs text-gray-400" onClick={(e) => { e.stopPropagation(); tableViewService.deleteView(v.id).then(loadViews); }}>×</span>
                                                </button>
                                            ))}
                                        </div>
                                        ...
                                        <h4 className="text-xs font-bold text-gray-500 uppercase mb-2 flex justify-between items-center">
                                            ...
                                            {showSaveView ? (
                                                <div className="flex gap-1">
                                                    <input className="w-20 border rounded px-1 text-xs" placeholder="Name" value={newViewName} onChange={e => setNewViewName(e.target.value)} />
                                                    <button onClick={handleSaveView} className="text-green-600"><Save className="w-3 h-3" /></button>
                                                </div>
                                            ) : (
                                                <button onClick={() => setShowSaveView(true)} className="text-blue-600 text-[10px] hover:underline">+ Save View</button>
                                            )}
                                        </h4>
                                        <div className="space-y-1">
                                            {columns.filter(c => c.id !== 'select').map(col => (
                                                <label key={col.id} className="flex items-center gap-2 text-sm text-gray-700 hover:bg-gray-50 px-1 py-0.5 rounded cursor-pointer">
                                                    <input type="checkbox" checked={col.visible} onChange={() => handleColumnToggle(col.id)} className="rounded text-blue-600 focus:ring-blue-500" />
                        ...
                        <div className="text-right">
                            ...
                            <div className="text-2xl font-bold text-blue-700 font-mono">{formatIndianNumber(subItem?.qty)} <span className="text-sm text-gray-400 font-normal">{subItem?.uom}</span></div>
                        </div>
                    </div>
                    ...
                    <div className="bg-blue-50/50 p-4 rounded-lg border border-blue-100 shadow-sm">
                        ...
                        <div className="grid grid-cols-12 gap-2 items-end">
                            ...
                            <div className="col-span-2">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">EPS Location</label>
                                <select
                                    className="w-full p-2 border border-gray-300 rounded text-sm focus:ring-1 focus:ring-blue-500"
                                    value={newM.epsNodeId}
                                    onChange={e => setNewM({ ...newM, epsNodeId: e.target.value })}
                                >
                            ...
                            <div className="col-span-2">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Element Name</label>
                                <input type="text" className="w-full p-2 border border-gray-300 rounded text-sm focus:ring-1 focus:ring-blue-500" placeholder="e.g. Wall A1" value={newM.elementName} onChange={e => setNewM({ ...newM, elementName: e.target.value })} />
                            </div>
                            <div className="col-span-2">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Category</label>
                                <input type="text" className="w-full p-2 border border-gray-300 rounded text-sm" placeholder="e.g. Wall" value={newM.elementCategory} onChange={e => setNewM({ ...newM, elementCategory: e.target.value })} />
                            </div>
                            <div className="col-span-2">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Type</label>
                                <input type="text" className="w-full p-2 border border-gray-300 rounded text-sm" placeholder="e.g. Concrete" value={newM.elementType} onChange={e => setNewM({ ...newM, elementType: e.target.value })} />
                            </div>
                            <div className="col-span-2">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Link Ref</label>
                                <input type="text" className="w-full p-2 border border-gray-300 rounded text-sm" placeholder="ID-123" value={newM.linkingElement} onChange={e => setNewM({ ...newM, linkingElement: e.target.value })} />
                            </div>
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Grid</label>
                                <input type="text" className="w-full p-2 border border-gray-300 rounded text-sm" placeholder="A-1" value={newM.grid} onChange={e => setNewM({ ...newM, grid: e.target.value })} />
                            </div>
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">UOM</label>
                                <input type="text" className="w-full p-2 border border-gray-300 rounded text-sm" placeholder="m3" value={newM.uom} onChange={e => setNewM({ ...newM, uom: e.target.value })} />
                            </div>

                            {/* Row 2: Dimensions & Technical */}
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Length</label>
                                <input type="number" className="w-full p-2 border border-gray-300 rounded text-sm" value={newM.length} onChange={e => setNewM({ ...newM, length: Number(e.target.value) })} />
                            </div>
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Breadth</label>
                                <input type="number" className="w-full p-2 border border-gray-300 rounded text-sm" value={newM.breadth} onChange={e => setNewM({ ...newM, breadth: Number(e.target.value) })} />
                            </div>
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Depth</label>
                                <input type="number" className="w-full p-2 border border-gray-300 rounded text-sm" value={newM.depth} onChange={e => setNewM({ ...newM, depth: Number(e.target.value) })} />
                            </div>
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Height</label>
                                <input type="number" className="w-full p-2 border border-gray-300 rounded text-sm" value={newM.height} onChange={e => setNewM({ ...newM, height: Number(e.target.value) })} />
                            </div>
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Bot Lvl</label>
                                <input type="number" className="w-full p-2 border border-gray-300 rounded text-sm" placeholder="0.00" value={newM.bottomLevel} onChange={e => setNewM({ ...newM, bottomLevel: e.target.value })} />
                            </div>
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Top Lvl</label>
                                <input type="number" className="w-full p-2 border border-gray-300 rounded text-sm" placeholder="3.00" value={newM.topLevel} onChange={e => setNewM({ ...newM, topLevel: e.target.value })} />
                            </div>
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Perim.</label>
                                <input type="number" className="w-full p-2 border border-gray-300 rounded text-sm" value={newM.perimeter} onChange={e => setNewM({ ...newM, perimeter: Number(e.target.value) })} />
                            </div>
                            <div className="col-span-1">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Base Area</label>
                                <input type="number" className="w-full p-2 border border-gray-300 rounded text-sm" value={newM.baseArea} onChange={e => setNewM({ ...newM, baseArea: Number(e.target.value) })} />
                            </div>
                            <div className="col-span-2">
                                <label className="text-[10px] font-bold text-gray-500 uppercase">Base Coords (JSON)</label>
                                <input type="text" className="w-full p-2 border border-gray-300 rounded text-sm font-mono text-xs" placeholder='[[0,0],[10,0]...]' value={newM.baseCoordinates} onChange={e => setNewM({ ...newM, baseCoordinates: e.target.value })} />
                            </div>

                            <div className="col-span-2 col-end-13">
                                <label className="text-[10px] font-bold text-gray-500 uppercase text-blue-600">Total Qty</label>
                                <div className="flex gap-1">
                                    <input type="number" className="w-full p-2 border-2 border-blue-200 rounded text-sm font-bold text-blue-700 bg-white" value={newM.qty} onChange={e => setNewM({ ...newM, qty: Number(e.target.value) })} />
                                    <button onClick={handleCalculate} className="px-2 py-1 bg-gray-200 rounded hover:bg-gray-300 text-gray-600" title="Calculate L*B*D">
                                        <Calculator className="w-4 h-4" />
                                    </button>
                                    <button onClick={handleAddManual} className="px-4 py-1 bg-blue-600 text-white rounded font-medium hover:bg-blue-700 shadow-sm ml-1">
                                        Add
                                    </button>
                                </div>
                            </div>
                    ...
                    <div className="flex-1 bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col overflow-hidden">
                            ...
                            <table className="w-full text-left text-sm whitespace-nowrap table-fixed">
                                ...
                                <thead className="bg-gray-100 text-gray-600 font-semibold sticky top-0 z-10 shadow-sm">
                                    <tr>
                                        {columns.filter(c => c.visible).map(col => (
                                            <th
                                                key={col.id}
                                                className={`p-3 border-b relative group ${col.align === 'right' ? 'text-right' : col.align === 'center' ? 'text-center' : 'text-left'} ${draggedColId === col.id ? 'bg-blue-100 opacity-50' : ''}`}
                                                draggable={col.id !== 'select'}
                                                onDragStart={(e) => handleDragStart(e, col.id)}
                                                onDragOver={(e) => handleDragOver(e, col.id)}
                                                onDrop={handleDrop}
                                                data-column-id={col.id} // Added for drop target identification
                                                style={{ cursor: col.id === 'select' ? 'default' : 'grab' }}
                                            >
                                                {col.id === 'select' ? (
                                                    <input
                                                        type="checkbox"
                                                    ...
                                                    />
                                                ) : <span className="truncate block select-none" title={col.label}>{col.label}</span>}
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {measurements.length === 0 ? (
                                        <tr>
                                            <td colSpan={columns.filter(c => c.visible).length} className="p-12 text-center text-gray-400">
                                                <div className="flex flex-col items-center gap-2">
                                                    <Calculator className="w-8 h-8 opacity-20" />
                                                    <p>No measurements recorded yet.</p>
                                                    <p className="text-xs">Use the form above or Import Excel to add quantities.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : (
                                                    ...
                                                    switch (col.id) {
                                                        case 'select': return (
                                                            <td key="select" className="p-3 border-b">
                                                                <input type="checkbox" checked={selectedIds.has(m.id)} onChange={() => toggleSelect(m.id)} className="rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
                                                            </td>
                                                        );
                                                        ...
                                                        case 'perimeter': return <td key={col.id} className="p-3 border-b text-right text-xs text-gray-500 font-mono">{formatIndianNumber(m.perimeter, 3)}</td>;
                                                        case 'baseArea': return <td key={col.id} className="p-3 border-b text-right text-xs text-gray-500 font-mono">{formatIndianNumber(m.baseArea, 3)}</td>;
                                                        case 'baseCoordinates': return <td key={col.id} className="p-3 border-b text-xs text-gray-400 truncate" title={JSON.stringify(m.baseCoordinates)}>{m.baseCoordinates ? 'Present' : '-'}</td>;
                                                        case 'plineAllLengths': return <td key={col.id} className="p-3 border-b text-xs text-gray-400 truncate" title={JSON.stringify(m.plineAllLengths)}>{m.plineAllLengths ? 'Present' : '-'}</td>;
                        ...
                        <div className="bg-gray-50 border-t border-gray-200 p-2 text-xs text-gray-500 flex justify-between px-4">
