...
export const WorkDocService = {
    ...
    // --- Import & Export ---
    previewExcelFn: async (projectId: number, file: File): Promise<{
        previewRows: string[][];
        ...
        const res = await api.post(`/workdoc/${projectId}/preview-excel`, formData, {
            headers: { 'Content-Type': 'multipart/form-data' }
        });
        return res.data;
    },

    importExcelFn: async (projectId: number, file: File, mapping: ColumnMapping, headerRow: number): Promise<ExcelImportResult> => {
        const formData = new FormData();
        formData.append('file', file);
    ...
    // --- Mapping ---
    getPendingVendorBoard: async (projectId: number): Promise<PendingBoardItem[]> => {
        const res = await api.get(`/workdoc/${projectId}/pending-vendor-board`);
        return res.data;
    },

    getMappingSuggestions: async (projectId: number, search: string) => {
        const res = await api.get(`/workdoc/mapping/suggestions`, { params: { projectId, search } });
        return res.data;
    },
    ...
    bulkMapItems: async (projectId: number, mappings: { woItemId: number; boqItemId: number | null }[]) => {
        const res = await api.post(`/workdoc/mapping/bulk`, { projectId, mappings });
        return res.data;
    },
};
