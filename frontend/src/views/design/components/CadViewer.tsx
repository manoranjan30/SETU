...
import { MousePointer2, Ruler, Loader2, RefreshCw, Layers, Crosshair, Settings2 } from 'lucide-react';
...
const CadViewer = ({ fileUrl }: CadViewerProps) => {
        ...
        try {
            ...
            if (dxf.tables && dxf.tables.layer) {
                Object.values(dxf.tables.layer.layers).forEach((layer: any) => {
                    ...
                    layersList.push({
                        name: layer.name,
                        visible: true,
                        color: layer.color ? `#${layer.color.toString(16).padStart(6, '0')}` : '#ffffff'
                    });
                });
            }
            ...
            if (dxf.entities) {
                dxf.entities.forEach((entity: any) => {
                    const color = entity.color ? `#${entity.color.toString(16).padStart(6, '0')}` : '#ffffff';
    ...
    const toggleLayer = (layerName: string) => {
        setLayers((prev: LayerState[]) => prev.map(l => {
            if (l.name === layerName) {
                const newVisible = !l.visible;
                const group = mainGroupRef.current?.children.find(c => c.name === layerName);
    ...
    const handleCanvasClick = (event: React.MouseEvent) => {
        ...
        if (mode === 'calibrate') {
            ...
            if (pts.length === 2) {
                ...
                const input = prompt("Enter the actual distance between these two points:");
                if (input && !isNaN(parseFloat(input))) {
                    const actualDist = parseFloat(input);
                    const worldDist = pts[0].distanceTo(pts[1]);
                    setScale(actualDist / worldDist);
                    alert(`Scale set: 1 unit = ${(actualDist / worldDist).toFixed(4)} real units`);
                }
            ...
            if (pts.length === 2) {
                ...
                sceneRef.current.add(createTextLabel(`${distance.toFixed(3)}`, pts[0].clone().lerp(pts[1], 0.5), zoom * 25));
                setMeasurePoints([]);
            }
        }
    };
    ...
    const addMarker = (position: THREE.Vector3, color: number) => {
        const zoom = (cameraRef.current!.top - cameraRef.current!.bottom) / 1000;
        const marker = new THREE.Mesh(new THREE.SphereGeometry(1, 8, 8), new THREE.MeshBasicMaterial({ color }));
        marker.scale.set(zoom * 5, zoom * 5, zoom * 5);
        marker.position.copy(position);
        sceneRef.current?.add(marker);
    };

    const createTextLabel = (text: string, position: THREE.Vector3, scale: number) => {
    ...
    return (
        <div className="w-full h-full flex flex-col relative overflow-hidden bg-[#1a1a1a]">
            ...
            <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
                <div className="bg-white/90 backdrop-blur rounded-lg shadow-xl p-1 flex flex-col gap-1 border border-white/20">
                    <ToolbarButton icon={<MousePointer2 size={18} />} active={mode === 'view'} onClick={() => setMode('view')} title="Pan Mode" />
                    <ToolbarButton icon={<Ruler size={18} />} active={mode === 'measure'} onClick={() => setMode('measure')} title="Measure" />
                    <ToolbarButton icon={<Settings2 size={18} />} active={mode === 'calibrate'} onClick={() => setMode('calibrate')} title="Calibrate Scale" />
                    <div className="h-px bg-gray-200 mx-1 my-0.5" />
                    <ToolbarButton icon={<Layers size={18} />} active={showLayers} onClick={() => setShowLayers(!showLayers)} title="Layers" />
                    <ToolbarButton icon={<Crosshair size={18} />} active={showCrosshair} onClick={() => setShowCrosshair(!showCrosshair)} title="Toggle Crosshair" />
                    <ToolbarButton icon={<RefreshCw size={18} />} onClick={() => { initThree(); loadDxf(); }} title="Reload" />
                </div>
            </div>

            {/* Layer Sidebar */}
            {showLayers && (
                <div className="absolute top-4 left-16 z-20 w-64 max-h-[80%] bg-white/95 backdrop-blur rounded-lg shadow-2xl border border-gray-200 overflow-hidden flex flex-col animate-in slide-in-from-left duration-200">
                    <div className="p-3 border-b bg-gray-50 flex justify-between items-center">
                        <span className="font-bold text-gray-700">Layers ({layers.length})</span>
                    </div>
                    <div className="flex-1 overflow-y-auto p-2">
                        {layers.map(layer => (
                            <div key={layer.name} className="flex items-center gap-2 p-1.5 hover:bg-gray-100 rounded cursor-pointer transition-colors" onClick={() => toggleLayer(layer.name)}>
                                <div className={`w-4 h-4 rounded-sm border ${layer.visible ? 'shadow-inner' : 'opacity-20'}`} style={{ backgroundColor: layer.color }} />
                                <span className={`text-sm truncate flex-1 ${layer.visible ? 'text-gray-800' : 'text-gray-400 line-through'}`}>{layer.name}</span>
                                <input type="checkbox" checked={layer.visible} readOnly className="rounded text-blue-600 focus:ring-0" />
                            </div>
                        ))}
                    </div>
                </div>
            ...
            {mode === 'calibrate' && (
                <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20 bg-amber-600 text-white px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 animate-bounce">
                    ...
                    <span className="text-sm border-l border-white/30 pl-3">
                        {calibratingPoints.length === 0 ? "Click start point of a known distance" : "Click end point"}
                    </span>
                </div>
            )}

            {/* Viewport Info */}
            <div className="absolute bottom-4 right-4 z-10 text-[10px] text-gray-400 font-mono bg-black/40 px-2 py-1 rounded">
            ...
            {loading && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#1a1a1a]/80 z-50 backdrop-blur-sm">
                    ...
                    <span className="mt-4 text-gray-300 font-medium tracking-widest text-xs uppercase">Rendering DXF Pipeline</span>
                </div>
            )}

            {error && (
                <div className="absolute inset-0 flex items-center justify-center bg-red-900/10 z-50">
                    <div className="bg-white p-6 rounded-xl shadow-2xl max-w-sm text-center border-t-4 border-red-500">
                        <h4 className="text-red-600 font-bold mb-2">Engine Error</h4>
...
const ToolbarButton = ({ icon, active, onClick, title }: any) => (
    <button
        onClick={onClick}
        className={`p-2.5 rounded-md transition-all ${active ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'}`}
        title={title}
    >
        {icon}
    </button>
