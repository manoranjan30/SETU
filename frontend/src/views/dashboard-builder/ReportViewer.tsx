...
import { FileText, Download, RefreshCw, ArrowLeft, Printer, LayoutGrid, Calendar } from 'lucide-react';
...
interface ReportConfig {
    ...
    columns: { key: string; label: string; format?: string; aggregate?: string }[];
    filters: any[];
    groupBy?: string[];
    createdAt: string;
}
...
export default function ReportViewer() {
    ...
    const loadReportData = async () => {
        ...
        try {
            ...
            const dataRes = await dashboardBuilderApi.queryData(dummyReport.dataSourceKey, {
    ...
    const columnDefs = useMemo(() => {
        ...
        return report.columns.map(c => ({
            ...
            valueFormatter: (params: any) => {
                ...
                if (!val) return '-';
                // Basic formatting
                if (c.key.toLowerCase().includes('date')) return new Date(val).toLocaleDateString();
    ...
    if (loading && !report) {
        return <div style={{ padding: 40, textAlign: 'center' }}>Initializing Report Builder...</div>;
    }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#f8fafc' }}>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                    <button
                        ...
                        style={{ padding: 8, borderRadius: 10, border: 'none', background: '#f1f5f9', cursor: 'pointer', color: '#64748b' }}
                    >
                        <ArrowLeft size={20} />
                    </button>
                    <div>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                    <button
                        ...
                        style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 10, border: '1px solid #e2e8f0', background: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer', color: '#475569' }}
                    >
                        <RefreshCw size={16} className={loading ? 'animate-spin' : ''} /> Run Again
                    </button>
                    <div style={{ width: 1, height: 32, background: '#e2e8f0', margin: '0 4px' }} />
                    <button
                        onClick={() => handleExport('EXCEL')}
                        style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 10, border: 'none', background: '#10b981', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}
                    >
                        <Download size={16} /> Excel
                    </button>
                    <button
                        onClick={() => exportUtils.toPdf()}
                        style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 10, border: 'none', background: '#0f172a', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}
                    >
            ...
            {/* Error States */}
            {refreshError && (
                <div style={{ margin: 24, padding: 16, background: '#fef2f2', border: '1px solid #fee2e2', borderRadius: 12, color: '#dc2626', fontSize: 14 }}>
            ...
            <div style={{ flex: 1, padding: 24, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                <div style={{
                    flex: 1, background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0',
                    ...
                    {rowData.length === 0 && !loading && (
                        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.8)' }}>
                            ...
                            <p style={{ color: '#94a3b8', margin: 0 }}>No matching records found for this report configuration.</p>
                        </div>
                    )}
                </div>

                <div style={{ marginTop: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center', color: '#64748b', fontSize: 13 }}>
                    <div>Total Records: <b>{rowData.length}</b></div>
                    <div>Source: <b>SETU Micro-Analysis Engine</b></div>
                </div>
            </div>
        </div>
    );
