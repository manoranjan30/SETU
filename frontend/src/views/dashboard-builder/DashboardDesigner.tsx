...
export default function DashboardDesigner() {
    ...
    setWidgets((prev) =>
      prev.map((w) => {
        const layoutItem = layout.find(
          (l: any) => l.i === String(w.id || w.sortOrder),
        );
        if (layoutItem) {
          return {
            ...w,
  ...
  const selectedDataSource = dataSources.find(
    (ds) => ds.key === selectedWidget?.dataSourceKey,
  );
    ...
    >
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 20 }}>
        {widgets.length === 0 ? (
          ...
          >
            ...
            <h3
              style={{ color: "#475569", margin: "0 0 8px", fontWeight: 600 }}
            >
              Start Building Your Dashboard
            </h3>
            <p style={{ color: "#94a3b8", margin: "0 0 16px", maxWidth: 360 }}>
              Click "Add Widget" to pick a chart type and data source, then drag
              and resize to arrange your layout
            </p>
            <button
              onClick={() => setShowAddPanel(true)}
              style={{
              ...
              >
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    <button
                      onClick={() =>
                        widget.id && setSelectedWidgetId(widget.id)
                      }
                      style={{
                        padding: 4,
                        border: "none",
      ...
      {selectedWidget && (
        <WidgetConfigPanel
          ...
          onUpdate={(updates) =>
            selectedWidget.id && handleUpdateWidget(selectedWidget.id, updates)
          }
