    ...
    return (
        <div style={{ background: '#fff', borderRadius: 12, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    <h3 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: '#1e293b' }}>Dashboard Assignments</h3>
                    <p style={{ margin: 0, fontSize: 12, color: '#94a3b8' }}>Define which roles or users see which dashboard by default</p>
                </div>
                <button
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                    <tr style={{ background: '#f8fafc', borderBottom: '1px solid #f1f5f9' }}>
                        <th style={{ padding: '12px 24px', textAlign: 'left', fontSize: 11, fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Scope / Target</th>
                        <th style={{ padding: '12px 24px', textAlign: 'left', fontSize: 11, fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Assigned Dashboard</th>
                        <th style={{ padding: '12px 24px', textAlign: 'center', fontSize: 11, fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Default</th>
                        <th style={{ padding: '12px 24px', textAlign: 'right', fontSize: 11, fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Actions</th>
                    </tr>
                </thead>
                <tbody>
                    {assignments.map((as) => (
                        <tr key={as.id} style={{ borderBottom: '1px solid #f8fafc' }}>
                            ...
                            <td style={{ padding: '16px 24px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    ...
                                    <span style={{ fontSize: 14, color: '#334155', fontWeight: 500 }}>{as.dashboard?.name || `ID: ${as.dashboardId}`}</span>
                                </div>
                            </td>
                            <td style={{ padding: '16px 24px', textAlign: 'center' }}>
                                {as.isDefault && <span style={{ padding: '2px 8px', borderRadius: 12, background: '#dcfce7', color: '#166534', fontSize: 11, fontWeight: 800 }}>ACTIVE</span>}
                            </td>
                            <td style={{ padding: '16px 24px', textAlign: 'right' }}>
                                <button
                                    onClick={() => handleDelete(as.id)}
                                    style={{ padding: 6, borderRadius: 6, border: 'none', background: 'transparent', color: '#94a3b8', cursor: 'pointer' }}
                                >
                                    <Trash2 size={16} />
                                </button>
                            </td>
                    ...
                    {assignments.length === 0 && !loading && (
                        <tr>
                            <td colSpan={4} style={{ padding: 40, textAlign: 'center', color: '#94a3b8', fontSize: 14 }}>No assignments defined yet.</td>
                        </tr>
                    )}
                </tbody>
            </table>
            ...
            {showAdd && (
                <div style={{
                    ...
                    <div style={{ background: '#fff', borderRadius: 16, width: 440, padding: 32, boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
                        <h3 style={{ margin: '0 0 24px', fontSize: 18, fontWeight: 800, color: '#1e293b' }}>New Assignment</h3>

                        <div style={{ marginBottom: 20 }}>
                            <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 8 }}>Scope Type</label>
                                ...
                                {['ROLE', 'USER', 'PROJECT', 'GLOBAL'].map(t => (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            borderColor: newAssignment.type === t ? '#2563eb' : '#e2e8f0',
                        ...
                        <div style={{ marginBottom: 20 }}>
                            <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#475569', marginBottom: 8 }}>Select Dashboard</label>
                            <select
                                value={newAssignment.dashboardId}
                                onChange={(e) => setNewAssignment({ ...newAssignment, dashboardId: Number(e.target.value) })}
                                style={{ width: '100%', padding: '12px', borderRadius: 8, border: '1px solid #e2e8f0', outline: 'none' }}
                            >
                                <option value={0}>Choose Dashboard...</option>
                                {dashboards.map(d => <option key={d.id} value={d.id}>{d.name} ({d.scope})</option>)}
                            </select>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: 12, marginTop: 32 }}>
                            <button
                                ...
                                style={{ flex: 1, padding: '12px', borderRadius: 8, border: '1px solid #e2e8f0', background: '#fff', color: '#475569', fontWeight: 600, cursor: 'pointer' }}
                            >
                                Cancel
                            </button>
                            <button
