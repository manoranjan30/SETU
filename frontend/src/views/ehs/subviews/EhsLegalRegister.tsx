...
const EhsLegalRegister: React.FC<Props> = ({ projectId }) => {
    ...
    const getStatusStyle = (expiryDate: string) => {
        ...
        if (expiry < today) return 'bg-red-50 text-red-700 font-bold'; // Row style for expired
        return '';
    };
    ...
    const handleDelete = async () => {
        if (!deleteId) return;
        try {
            await api.delete(`/ehs/legal/${deleteId}`);
            setDeleteId(null);
            setDeleteStep(0);
            fetchData();
        } catch (error) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (editingId) {
                ...
                await api.post(`/ehs/${projectId}/legal`, payload);
            }
            setShowModal(false);
            setEditingId(null);
            resetForm();
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
                    <div>
                        <p className="text-sm text-gray-500 mb-1">Expiring Soon (30 Days)</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-6 border-b flex justify-between items-center">
                    <h3 className="font-bold text-gray-900">Legal Compliance Register</h3>
                    <button
                            ...
                            {data.map((item, index) => {
                                ...
                                return (
                                    <tr key={item.id} className={`hover:bg-gray-50/50 ${rowClass}`}>
                                        ...
                                        <td className="px-6 py-4 font-medium">{item.requirement}</td>
                                        ...
                                        <td className="px-6 py-4">
                                            {item.certifiedDate ? new Date(item.certifiedDate).toLocaleDateString('en-GB') : '-'}
                                        </td>
                                        <td className="px-6 py-4 font-medium">
                                            {item.expiryDate ? new Date(item.expiryDate).toLocaleDateString('en-GB') : 'Perpetual'}
                                        </td>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="px-6 py-4 border-b flex justify-between items-center bg-gray-50">
                            <h3 className="font-bold text-lg">{editingId ? 'Update' : 'Add'} Compliance Data</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1">Responsibility</label>
                                    <select
                                        className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-600/20 outline-none"
                                ...
                                <div>
                                    {/* Status is auto-calculated mostly, but allow manual override if needed or just display dummy for now since backend stores it */}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-1">Certified Date</label>
                                    <input
                                        type="date"
                                        className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-600/20 outline-none"
                                        value={formData.certifiedDate}
                                        onChange={(e) => setFormData({ ...formData, certifiedDate: e.target.value })}
                                    />
                                </div>
                                <div>
                                    ...
                                    />
                                    <p className="text-xs text-gray-400 mt-1">Leave blank if perpetual</p>
                                </div>
                            </div>
                            <div>
                ...
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white w-full max-w-sm rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 p-6 text-center">
                        ...
                        <h3 className="text-lg font-bold text-gray-900 mb-2">
                            {deleteStep === 1 ? 'Delete Compliance Record?' : 'Are you absolutely sure?'}
                        </h3>
                        <p className="text-sm text-gray-500 mb-6">
                            {deleteStep === 1
                                ? 'This action will remove this compliance record permanently.'
...
export default EhsLegalRegister;
