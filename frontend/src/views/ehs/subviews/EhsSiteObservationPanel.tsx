...
const EhsSiteObservationPanel: React.FC<SiteObservationPanelProps> = ({
  projectId,
}) => {
  ...
  >(() => {
    try {
      return JSON.parse(localStorage.getItem("ehsObservationExportPresets") || "[]");
    } catch {
      return [];
    }
  });
  ...
  const fetchRecords = async () => {
    setLoading(true);
    try {
      const resp = await api.get(`/ehs/site-observations`, {
        params: {
          projectId,
          paged: true,
          page,
  ...
  const fetchCategories = async () => {
    try {
      const resp = await api.get(`/ehs/site-observations/categories/${projectId}`);
      setCategories(Array.isArray(resp.data) ? resp.data : []);
    } catch (error) {
      console.error("Failed to fetch EHS observation categories", error);
    }
  };
  ...
  const saveCategories = async (nextCategories: string[]) => {
    ...
    try {
      const resp = await api.put(
        `/ehs/site-observations/categories/${projectId}`,
        { categories: nextCategories },
      );
      setCategories(Array.isArray(resp.data) ? resp.data : nextCategories);
      return true;
    } catch (error) {
      console.error("Failed to update EHS observation categories", error);
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post("/ehs/site-observations", payload);
      ...
      setFormData({
        ...
        locationLabel: "",
        severity: "MINOR",
        category: categories[0] || "General Safety",
        description: "",
        remarks: "",
        targetDate: "",
      });
  ...
  const handleRectifySubmit = async (e: React.FormEvent) => {
    ...
    try {
      const photoUrls = await uploadFiles(photos);
      await api.patch(
        `/ehs/site-observations/${selectedRecord.id}/rectify`,
        {
          rectificationText,
  ...
  const handleCloseSubmit = async () => {
    ...
    setUploading(true);
    try {
      await api.patch(`/ehs/site-observations/${selectedRecord.id}/close`, {
        closureRemarks,
      });
  ...
  const handleRejectRectification = async () => {
    ...
    try {
      await api.patch(
        `/ehs/site-observations/${selectedRecord.id}/reject-rectification`,
        { rejectionRemarks },
      );
      setShowCloseModal(false);
      setClosureRemarks("");
  ...
  const handleHoldObservation = async () => {
    ...
    setUploading(true);
    try {
      await api.patch(`/ehs/site-observations/${selectedRecord.id}/hold`, {
        holdReason,
      });
      fetchRecords();
      setShowCloseModal(false);
  ...
  const handleUnholdObservation = async () => {
    ...
    setUploading(true);
    try {
      await api.patch(`/ehs/site-observations/${selectedRecord.id}/unhold`);
      fetchRecords();
      setShowCloseModal(false);
    } catch (error: any) {
  ...
  const handleDelete = async (id: string) => {
    if (
      ...
      return;
    try {
      await api.delete(`/ehs/site-observations/${id}`);
      fetchRecords();
    } catch (error: any) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const response = await api.get(
        `/ehs/site-observations/export?${params.toString()}`,
        { responseType: "blob" },
      );
      const url = URL.createObjectURL(response.data);
      const link = document.createElement("a");
      link.href = url;
      link.download = `EHS_Observation_Register_${new Date()
        .toISOString()
        .slice(0, 10)}.xlsx`;
  ...
  const fetchExportPresets = async () => {
    try {
      const response = await api.get("/export-presets", {
        params: { module: "EHS", tableKey: "site-observations" },
      });
      ...
      localStorage.setItem("ehsObservationExportPresets", JSON.stringify(presets));
    } catch (error) {
  ...
  const saveExportPreset = async () => {
    ...
    try {
      ...
      localStorage.setItem("ehsObservationExportPresets", JSON.stringify(next));
    } catch (error) {
      const next = [
        preset,
        ...exportPresets.filter(
...
export default EhsSiteObservationPanel;
