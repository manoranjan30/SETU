...
const EhsManhours: React.FC<Props> = ({ projectId }) => {
    ...
    try {
      ...
      if (response.data.length > 0) {
        ...
        const prevTotal = prevRecords.reduce(
          (sum: number, r: any) => sum + Number(r.safeManhours),
          0,
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const staffMandays =
      (Number(formData.staffMale) + Number(formData.staffFemale)) *
      (laborStats.workingDays || 26);
    const totalWorkerMandays = laborStats.totalWorkerManDays;

    const totalManhours =
      (staffMandays + totalWorkerMandays) * Number(formData.avgWorkHours);
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          {
            label: "Total Cumulative (Million)",
            value: (stats.cumulative / 1000000).toFixed(2) + " M",
          },
        ].map((kpi, i) => (
          <div
            key={i}
            className="bg-surface-card p-6 rounded-2xl border border-border-subtle shadow-sm"
          >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y">
              {data.map((row) => (
                <tr key={row.id} className="hover:bg-surface-base/50">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-surface-card w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-2 bg-surface-raised rounded-lg hover:bg-gray-200 text-text-secondary text-xs font-bold whitespace-nowrap"
                  >
                    Sync from Manpower
                  </button>
                </div>
                <div className="mt-3 p-3 bg-surface-base rounded-lg text-xs text-text-secondary flex flex-col gap-1 border border-border-default">
                  ...
                  <div className="text-[10px] text-text-disabled mt-1 italic">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        staffMale: Number(e.target.value),
                      })
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        staffFemale: Number(e.target.value),
                      })
                    }
                  />
                </div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          avgWorkHours: Number(e.target.value),
                        })
                      }
                    />
                    <span className="absolute right-3 top-2 text-xs text-text-disabled font-medium">
                      Hrs/Man
                    </span>
                  </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        ltiDeductions: Number(e.target.value),
                      })
                    }
                  />
                </div>
              ...
              <div className="p-4 bg-primary-muted rounded-xl text-sm text-blue-800 space-y-1">
                <div className="flex justify-between">
                  <span>Staff ManDays:</span>
                  <span className="font-bold">
                    {(formData.staffMale + formData.staffFemale) *
                      (laborStats.workingDays || 0)}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Worker ManDays:</span>
                  <span className="font-bold">
                    {laborStats.totalWorkerManDays}
                  </span>
                </div>
                <div className="pt-2 border-t border-blue-200 flex justify-between font-bold">
                  ...
                  <span>
                    {((formData.staffMale + formData.staffFemale) *
                      (laborStats.workingDays || 0) +
                      Number(laborStats.totalWorkerManDays)) *
                      formData.avgWorkHours -
                      formData.ltiDeductions}
                  </span>
                </div>
              </div>
