...
import { Lightbulb, TrendingUp, AlertTriangle, Target, Briefcase } from 'lucide-react';
...
const EfficiencyInsights: React.FC<EfficiencyInsightsProps> = ({ data, loading }) => {
    if (loading) {
        return (
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 h-full flex flex-col">
            <div className="flex justify-between items-start mb-6">
                <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">AI-driven suggestions for better planning</p>
                </div>
            </div>
            ...
            <div className="flex-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar">
                    ...
                    <div className="space-y-3">
                        {data.topBurners.slice(0, 3).map((item, idx) => (
                            <div key={idx} className="flex justify-between items-center text-sm">
                                <span className="text-slate-600 truncate flex-1 pr-4" title={item.name}>{item.name}</span>
                ...
                {data.alerts.length > 0 ? (
                    data.alerts.map((alert, idx) => (
                        <div key={idx} className={`p-4 rounded-xl border ${alert.type === 'warning' ? 'bg-amber-50 border-amber-100 text-amber-800' : 'bg-sky-50 border-sky-100 text-sky-800'}`}>
                            <div className="flex gap-3">
                                {alert.type === 'warning' ? <AlertTriangle className="w-5 h-5 flex-shrink-0" /> : <Briefcase className="w-5 h-5 flex-shrink-0" />}
                                <div>
                                    <p className="text-sm font-bold mb-1">{alert.type === 'warning' ? 'Attention Required' : 'Planning Tip'}</p>
                    ...
                    <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100 text-emerald-800 flex gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs opacity-90">Progress aligns perfectly with the plan. No critical alerts.</p>
                        </div>
                    </div>
                )}
            </div>
