    ...
    return (
      <div className="grid grid-cols-4 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className="bg-surface-card rounded-2xl p-6 shadow-sm border border-border-default animate-pulse h-32"
          >
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      {cards.map((card, idx) => (
        <div
          key={idx}
          className="bg-surface-card rounded-2xl p-6 shadow-sm border border-border-default hover:shadow-md transition-shadow relative overflow-hidden group"
        >
          <div className="relative z-10">
            <div className="flex justify-between items-start mb-4">
              ...
              <div
                className={`p-3 rounded-xl ${card.bgColor} ${card.textColor}`}
              >
            ...
            <div className="flex items-center gap-1.5 mt-2">
              <div className="h-1.5 flex-1 bg-slate-100 rounded-full overflow-hidden">
                <div
                  className={`h-full ${card.color} w-3/4 rounded-full opacity-20`}
                ></div>
              </div>
            </div>
            <p className="text-[10px] text-text-disabled font-medium mt-2">
              {card.desc}
            </p>
          </div>
          {/* Background Pattern */}
