...
export default function QualityStructureManager({ projectId }: Props) {
  ...
  const [floorStructure, setFloorStructure] =
    useState<QualityFloorStructure | null>(null);
  ...
  const [selectedTargetFloorIds, setSelectedTargetFloorIds] = useState<
    number[]
  >([]);
  ...
  const [namingMode, setNamingMode] = useState<
    "KEEP" | "FLOOR_PREFIX_REMAP" | "REPLACE_PREFIX"
  >("FLOOR_PREFIX_REMAP");
  ...
  const [newRoomDraftByUnit, setNewRoomDraftByUnit] = useState<
    Record<number, RoomDraft>
  >({});
    ...
    return allFloors
      ...
      .filter(
        (f) =>
          getNodeName(f)
            .toLowerCase()
            .includes(targetFloorSearch.toLowerCase()) ||
  ...
  const copyPreview = useMemo(() => {
    ...
    filteredTargetFloors,
    selectedTargetFloorIds,
    floorStructure?.units,
    namingMode,
    selectedNode,
    sourcePrefix,
  ]);
  ...
  const handlePreview = async () => {
    if (!selectedNode || getNodeType(selectedNode) !== "FLOOR") return;
    try {
      const data = await qualityService.previewBuild(
        projectId,
        selectedNode.id,
        {
          unitCount,
  ...
  const handleCopy = async () => {
    ...
    const targetFloorIds = selectedTargetFloorIds.filter(
      (v) => v !== selectedNode.id,
    );
  ...
  return (
    <div className="flex h-[calc(100vh-240px)] bg-surface-card border rounded-xl shadow-sm overflow-hidden">
          ...
          <div className="h-full flex items-center justify-center text-text-muted">
            <div className="text-center">
              <div className="font-semibold">
                Selected: {getNodeName(selectedNode)} (
                {getNodeType(selectedNode)})
              </div>
              <div className="text-sm mt-2">
                Please select a FLOOR node to configure units and rooms.
              </div>
            </div>
          </div>
        ) : (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="border border-border-default rounded-xl p-4 bg-surface-card">
                <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2 text-sm uppercase tracking-wider">
                  <Wand2 className="w-4 h-4 text-secondary" /> Build Units (Per
                  Floor)
                </h3>
                ...
                <div className="mt-3">
                  <label className="block text-xs font-bold text-text-muted uppercase mb-1">
                    Default Rooms (comma separated)
                  </label>
                  <textarea
                    value={defaultRooms}
                    onChange={(e) => setDefaultRooms(e.target.value)}
                    className="w-full border border-border-default rounded-lg px-3 py-2 text-sm h-20"
                  />
                </div>
                ...
                <div className="mt-4 flex gap-2">
                  <button
                    ...
                    className="px-4 py-2 bg-secondary text-white rounded-lg text-sm font-semibold hover:bg-secondary-dark"
                  >
                    Generate Preview
                  </button>
                  <button
                    onClick={handleApply}
                    disabled={saving || previewUnits.length === 0}
                    className="px-4 py-2 bg-green-600 text-white rounded-lg text-sm font-semibold hover:bg-green-700 disabled:opacity-50"
                  >
                    {saving ? "Applying..." : "Create Units & Rooms"}
                  </button>
                </div>
              </div>
              ...
              <div className="border border-border-default rounded-xl p-4 bg-surface-card">
                <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2 text-sm uppercase tracking-wider">
                  <Copy className="w-4 h-4 text-secondary" /> Copy To Other
                  Floors
                </h3>
                ...
                <div className="text-xs text-text-muted mb-2 flex gap-2">
                  <button
                    type="button"
                    className="underline"
                    onClick={() =>
                      setSelectedTargetFloorIds(
                        filteredTargetFloors.map((f) => f.id),
                      )
                    }
                  >
                    Select all filtered
                ...
                <div className="max-h-36 overflow-auto border border-border-default rounded-lg p-2 space-y-1 mb-3">
                  {filteredTargetFloors.map((f) => (
                    <label
                      key={f.id}
                      className="flex items-center gap-2 text-sm cursor-pointer hover:bg-surface-base p-1 rounded"
                    >
                      <input
                        type="checkbox"
                ...
                <Field label="Naming Mode">
                  <select
                    ...
                    className="w-full border border-border-default rounded-lg px-3 py-2 text-sm"
                  >
                    <option value="FLOOR_PREFIX_REMAP">
                      Floor Prefix Remap (101 -&gt; 201)
                    </option>
                ...
                <Field label="Collision Mode">
                  <select
                    value={collisionMode}
                    onChange={(e) => setCollisionMode(e.target.value as any)}
                    className="w-full border border-border-default rounded-lg px-3 py-2 text-sm"
                  >
                    <option value="REPLACE">
                      Replace existing target structure
                    </option>
                    <option value="SKIP">
                      Skip floors with existing structure
                    </option>
                    <option value="FAIL">
                      Fail if target has existing structure
                    </option>
                  </select>
                </Field>
                    ...
                    >
                      <div className="col-span-3">
                        <input
                          ...
                          className="w-full border border-border-default rounded-lg px-2 py-1.5 text-sm"
                        />
                      </div>
              ...
              {(floorStructure?.units?.length || 0) === 0 ? (
                ...
                <div className="space-y-2">
                  {floorStructure?.units.map((u) => (
                    <div
                      key={u.id}
                      className="p-3 border border-border-subtle rounded-lg"
                    >
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="border border-border-default rounded px-2 py-1 text-sm font-medium text-text-primary flex-1"
                        />
                        <button
                          onClick={() => saveUnit(u.id)}
                          disabled={structureSaving}
                          className="px-2 py-1 text-xs rounded bg-primary text-white hover:bg-primary-dark disabled:opacity-50"
                        >
                          Save Unit
                        </button>
                        <button
                        ...
                        {u.rooms.map((r) => (
                          ...
                          >
                            <input
                              ...
                              className="col-span-5 border border-border-default rounded px-2 py-1 text-sm"
                              placeholder="Room name"
                            />
                            <input
                              value={
                                roomDrafts[r.id]?.roomType ?? (r.roomType || "")
                              }
                              onChange={(e) =>
                                setRoomDrafts((prev) => ({
                                  ...prev,
                              ...
                              className="col-span-3 border border-border-default rounded px-2 py-1 text-sm"
...
function TreeNode({
  ...
  return (
    <div className="ml-3 select-none">
      <div
        className={`flex items-center cursor-pointer p-1.5 rounded-lg transition-all ${selectedId === node.id ? "bg-primary-muted text-blue-700 font-medium" : "hover:bg-surface-raised text-text-secondary"}`}
        onClick={() => {
          onSelect(node);
      ...
      >
        <button
          ...
          className={`mr-1 p-0.5 rounded hover:bg-black/5 ${hasChildren ? "text-text-disabled" : "invisible"}`}
        >
          {expanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
        </button>
        <Layers className="w-3.5 h-3.5 mr-1" />
        <span className="truncate text-sm">
          {getNodeName(node)} ({getNodeType(node)})
        </span>
      </div>
      {expanded && hasChildren && (
        <div className="ml-2 border-l border-border-default pl-1">
          {node.children!.map((c) => (
            <TreeNode
              key={c.id}
              node={c}
