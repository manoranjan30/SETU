...
import { Plus, Edit2, Trash2, X, Search, FlaskConical, Calendar, Package } from 'lucide-react';
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-4 bg-white px-4 py-2 rounded-xl border border-gray-100 shadow-sm w-96">
                    ...
                    <input type="text" placeholder="Search material tests..." className="bg-transparent border-none focus:ring-0 text-sm w-full" />
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {tests.map((test) => (
                    <div key={test.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 hover:shadow-md transition-all relative group">
                        <div className="flex justify-between items-start mb-4">
                            <div className={`p-2 rounded-lg ${test.result === 'Pass' ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                                <FlaskConical className="w-6 h-6" />
                            </div>
                            <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => openEditModal(test)} className="p-1.5 hover:bg-gray-100 rounded-lg text-gray-500"><Edit2 className="w-3.5 h-3.5" /></button>
                        ...
                        <h4 className="font-bold text-gray-900 text-lg">{test.materialName}</h4>
                        <p className="text-sm text-gray-500 mb-4">{test.testType}</p>

                        <div className="space-y-2 mb-6">
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-400 flex items-center gap-1.5"><Package className="w-3.5 h-3.5" /> Batch No.</span>
                                <span className="font-medium text-gray-700">{test.batchNumber}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-400 flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> Test Date</span>
                                <span className="font-medium text-gray-700">{test.testDate}</span>
                            </div>
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-400">Result</span>
                                <span className={`font-bold ${test.result === 'Pass' ? 'text-emerald-600' : 'text-red-600'}`}>{test.result}</span>
                            </div>
                        </div>

                        <div className={`mt-auto pt-3 border-t border-gray-50 flex items-center justify-between`}>
                            <span className={`text-xs font-bold px-2 py-1 rounded-full ${test.status === 'Approved' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
                                {test.status}
                            </span>
                            <span className="text-[10px] text-gray-400 uppercase font-black">{test.supplier}</span>
                        </div>
                    </div>
                ))}
            </div>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in duration-200">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Batch / Tag Number</label>
                                <input
                                    type="text" required
                                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-orange-500"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Received Date</label>
                                <input
                                    type="date" required
                                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-orange-500"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Test Date</label>
                                <input
                                    type="date" required
                                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-orange-500"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Test Type</label>
                                <select
                                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-orange-500"
                                    value={formData.testType}
                                    onChange={(e) => setFormData({ ...formData, testType: e.target.value })}
                                >
                                    <option value="Cube Test">Cube Test (Compressive Strength)</option>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Result</label>
                                <select
                                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-orange-500"
                            ...
                            <div className="col-span-2">
                                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Test Parameters / Remarks</label>
                                <textarea
                                    rows={3}
                                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-orange-500"
                                    value={formData.testParameters}
                                    onChange={(e) => setFormData({ ...formData, testParameters: e.target.value })}
                                />
                            </div>
