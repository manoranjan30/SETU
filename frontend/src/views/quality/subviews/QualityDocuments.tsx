...
import { Plus, Edit2, Trash2, X, Search, FileText, Download, Calendar, HardDrive } from 'lucide-react';
        ...
        <div className="space-y-6 text-gray-900">
            <div className="flex justify-between items-center">
                <div className="flex items-center gap-4 bg-white px-4 py-2 rounded-xl border border-gray-100 shadow-sm w-96">
                    ...
                    <input type="text" placeholder="Search quality documents..." className="bg-transparent border-none focus:ring-0 text-sm w-full" />
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {docs.map((doc) => (
                    <div key={doc.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 hover:shadow-md transition-all flex items-start gap-4 group">
                        ...
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">• {doc.documentType}</span>
                            </div>
                            <h4 className="font-bold text-gray-900 truncate">{doc.documentName}</h4>
                            <p className="text-xs text-gray-400 font-medium mb-3">{doc.referenceNumber}</p>
                            ...
                            <div className="flex items-center gap-4 border-t border-gray-50 pt-3">
                                ...
                                <div className={`text-[10px] font-black uppercase px-2 py-0.5 rounded ${doc.status === 'Approved' ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'}`}>
                                    {doc.status}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-2">
                            <button className="p-2 hover:bg-orange-50 text-orange-600 rounded-lg transition-colors"><Download className="w-4 h-4" /></button>
                            <div className="opacity-0 group-hover:opacity-100 transition-all flex flex-col gap-1">
                                <button onClick={() => openEditModal(doc)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-blue-600"><Edit2 className="w-4 h-4" /></button>
                                <button onClick={() => handleDelete(doc.id)} className="p-2 hover:bg-gray-100 rounded-lg text-gray-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in duration-200">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 grid grid-cols-2 gap-4 text-gray-900">
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Reference Number</label>
                                <input
                                    type="text" required
                                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-orange-500"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Submission Date</label>
                                <input
                                    type="date"
                                    className="w-full bg-gray-50 border-none rounded-xl px-4 py-3 text-sm font-medium focus:ring-2 focus:ring-orange-500"
