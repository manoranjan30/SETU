...
function SequencerContent() {
    const { listId, projectId } = useParams<{ listId: string; projectId: string }>();
    ...
    const onDrop = useCallback(
        async (event: React.DragEvent) => {
            ...
            try {
                const newActivity = await qualityService.createActivity(Number(listId), { activityName: name });

                const newNode: Node = {
                    id: String(newActivity.id),
                    type,
                    position,
                    data: { label: name, sequence: newActivity.sequence, description: newActivity.description },
                };
    ...
    if (loading) return <div className="p-10 text-center flex items-center justify-center h-full">Loading sequencer...</div>;

    return (
        <div className="dndflow h-screen w-full flex flex-col">
            <div className="bg-white border-b px-6 py-3 flex justify-between items-center z-10 shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-slate-800">Quality Workflow Editor</h1>
                        <p className="text-xs text-slate-500">Drag connections to define inspection sequence</p>
                    </div>
                </div>
                <div className="flex gap-4">
                    <div className="flex bg-gray-100 p-1 rounded-lg">
                        <button
                            onClick={() => navigate(`/dashboard/projects/${projectId}/quality/activity-lists/${listId}/activities`)}
                    ...
                    <button
                        onClick={onSave}
                        disabled={saving}
                        className="bg-blue-600 hover:bg-blue-700 disabled:opacity-70 text-white px-4 py-2 rounded shadow-sm font-medium flex items-center gap-2 transition-colors"
                    >
            ...
            <div className="flex flex-1 overflow-hidden" ref={reactFlowWrapper}>
                ...
                <div className="flex-1 bg-slate-50 relative h-full">
                    ...
                    <div className="absolute top-4 right-4 bg-white/95 backdrop-blur shadow-lg p-4 rounded-xl text-sm border border-slate-200 w-64 select-none pointer-events-none">
                        <h3 className="font-bold mb-3 text-slate-800 border-b pb-2">Constraints Legend</h3>
                        <div className="flex items-center gap-3 mb-2">
                            <div className="w-8 h-1 bg-red-500 rounded-full"></div>
                            <span className="font-medium text-slate-700">Hard (Unbreakable)</span>
                        </div>
                        <div className="text-xs text-slate-500 mb-3 ml-11">
                        ...
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="font-medium text-slate-700">Soft (Warning)</span>
                        </div>
                        <div className="text-xs text-slate-500 ml-11">
                            Shows warning but allows bypass with a reason.
                        </div>

                        <div className="mt-4 pt-3 border-t text-xs text-slate-400 italic">
