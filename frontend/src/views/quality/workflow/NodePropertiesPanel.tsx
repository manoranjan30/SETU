...
const NodePropertiesPanel: React.FC<NodePropertiesPanelProps> = ({
  ...
  const handleChange = (
    ...
    let value: any =
      e.target.type === "checkbox"
        ...
        : e.target.value;
    if (
      e.target.name === "assignedUserId" ||
      e.target.name === "assignedRoleId" ||
      e.target.name === "stepOrder"
    ) {
  ...
  return (
    <div className="h-full flex flex-col pt-16">
      <div className="border-b px-6 py-4 bg-surface-base flex items-center justify-between">
      ...
      <div className="flex-1 overflow-y-auto w-full max-h-screen pb-32">
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex bg-surface-raised p-1 rounded-md mb-3">
              <button
                className={`flex-1 py-1 text-sm rounded-sm ${data.assignmentMode === "ROLE" ? "bg-surface-card shadow-sm font-semibold" : "text-text-muted"}`}
                onClick={() =>
                  onChange({ assignmentMode: "ROLE", assignedUserId: null })
                }
              >
                Role
              </button>
              <button
                className={`flex-1 py-1 text-sm rounded-sm ${data.assignmentMode === "USER" ? "bg-surface-card shadow-sm font-semibold" : "text-text-muted"}`}
                onClick={() =>
                  onChange({ assignmentMode: "USER", assignedRoleId: null })
                }
              >
                Specific User
              </button>
          ...
          <div className="border-t pt-4">
            ...
            <div className="space-y-3">
              {[
                ...
                {
                  name: "allowFinalApprove",
                  label: "Final Approval (Digital Lock)",
                },
                ...
                <label
                  key={opt.name}
                  className="flex items-center space-x-3 text-sm text-text-secondary cursor-pointer"
                >
                  <input
                    type="checkbox"
                    name={opt.name}
                    ...
                    onChange={handleChange}
                    className="h-4 w-4 text-primary border-border-strong rounded focus:ring-black accent-black"
                  />
                  <span>{opt.label}</span>
                </label>
              ))}
