...
export class CreateMicroScheduleDto {
    ...
    @ApiProperty({ description: 'List of Master Activity IDs included in this Micro Schedule', type: [Number], required: false })
    @IsOptional()
    @IsNumber({}, { each: true })
    ...
    @ApiProperty({ description: 'Status', enum: MicroScheduleStatus, required: false })
    @IsOptional()
