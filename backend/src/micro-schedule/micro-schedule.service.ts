...
import { MicroSchedule, MicroScheduleStatus } from './entities/micro-schedule.entity';
...
import { MicroScheduleActivity, MicroActivityStatus } from './entities/micro-schedule-activity.entity';
...
export class MicroScheduleService {
    ...
    async create(
        ...
        // Process Linked Activities (Multi-Select)
        if (dto.linkedActivityIds && dto.linkedActivityIds.length > 0) {
            const linkedActivities = await this.activityRepo.find({
                where: { id: In(dto.linkedActivityIds) },
                relations: ['wbsNode'],
            });

            const microActivities: MicroScheduleActivity[] = [];
            for (const act of linkedActivities) {
                const microAct = this.microActivityRepo.create({
                    microSchedule: saved,
                    microScheduleId: saved.id,
                    parentActivityId: act.id,
                    name: act.activityName,
                    uom: '-', // Default UOM
                    allocatedQty: 1, // Default Quantity (to be refined)
                    plannedStart: act.startDatePlanned || saved.plannedStart,
                    plannedFinish: act.finishDatePlanned || saved.plannedFinish,
                    status: MicroActivityStatus.PLANNED,
                    epsNodeId: act.projectId, // Assuming projectId is valid EPS Node
                });
                microActivities.push(microAct);
            }

            if (microActivities.length > 0) {
                await this.microActivityRepo.save(microActivities);
            }
        }
     ...
     */
    async findByParentActivity(parentActivityId: number): Promise<MicroSchedule[]> {
        return await this.microScheduleRepo.find({
    ...
    async updateOvershootFlag(id: number): Promise<void> {
        ...
        if (!microSchedule.forecastFinish || !microSchedule.parentActivity || !microSchedule.parentActivity.finishDatePlanned) {
            return;
        }

        const forecastDate = new Date(microSchedule.forecastFinish);
        const parentFinishDate = new Date(microSchedule.parentActivity.finishDatePlanned);
    ...
    async recalculateTotals(id: number): Promise<void> {
        ...
        if (microSchedule.activities) {
            for (const activity of microSchedule.activities as any[]) {
