...
async function bootstrap() {
  ...
  `);
    ...
    console.log(`Found ${ghostLedgers.length} ghost ledgers with negative balance.`);

    for (const ledger of ghostLedgers) {
        console.log(`Processing ledger for Activity ID: ${ledger.parentActivityId}, BOQ ID: ${ledger.boqItemId}`);
