...
@Injectable()
export class EhsObservationExportCron {
  private readonly logger = new Logger(EhsObservationExportCron.name);

  constructor(
    private readonly settings: SystemSettingsService,
    private readonly emailDelivery: EmailDeliveryService,
    private readonly observations: EhsObservationService,
  ...
  async sendWeeklyExports() {
    if (
      !(await this.settings.getSettingBool(
        'EHS_OBSERVATION_WEEKLY_EXPORT_ENABLED',
      ))
    ) {
      return;
    }
    const frequency =
      (await this.settings.getSetting('EHS_OBSERVATION_EXPORT_FREQUENCY')) ||
      'WEEKLY';
    ...
    const recipients = this.csv(
      await this.settings.getSetting('EHS_OBSERVATION_WEEKLY_EXPORT_RECIPIENTS'),
    );
    const projectIds = this.csv(
      await this.settings.getSetting('EHS_OBSERVATION_WEEKLY_EXPORT_PROJECT_IDS'),
    )
      .map(Number)
      .filter(Number.isFinite);

    if (recipients.length === 0 || projectIds.length === 0) {
      this.logger.warn('Weekly EHS observation export skipped: recipients or project IDs missing.');
    ...
    for (const projectId of projectIds) {
      ...
      const fileName = `EHS_Observation_Register_${projectId}_${to}.xlsx`;
      try {
        ...
        await this.emailDelivery.sendMail({
          to: recipients,
          subject: `SETU EHS Observation Register - Project ${projectId}`,
          text: 'Attached is the weekly EHS observation register export for the last 7 days.',
          attachments: [
            {
              filename: fileName,
              content: buffer,
        ...
        this.logger.error(`Weekly EHS observation export failed for project ${projectId}: ${message}`);
        await this.exportHistory.record({
          module: 'EHS',
          exportType: 'WEEKLY_OBSERVATION_REGISTER',
          projectId,
