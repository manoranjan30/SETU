...
export class QualityRatingService {
  ...
  async updateConfig(
    projectId: number,
    update: Partial<QualityRatingConfig>,
  ...
  async calculateProjectRating(
    ...
    const weightedObservationScore =
      (contextWeights.observations / 5) * averageObservationScore;
    const weightedDocScore =
      (contextWeights.documentation / 5) * documentationScore;
    const weightedCustScore =
      (contextWeights.customerInspections / 5) * customerInspectionScore;

    const overallScore =
      weightedObservationScore +
      weightedDocScore +
      weightedCustScore -
      deductionPoints;
  ...
  async saveMonthlyRating(
    ...
    let rating = await this.ratingRepo.findOne({
      where: { projectNodeId: projectId, period },
    });
    if (!rating) {
