...
export class QualityReportService {
    ...
    async generateInspectionReport(inspectionId: number): Promise<Buffer> {
        ...
        return new Promise((resolve, reject) => {
                ...
                for (const stage of inspection.stages) {
                    ...
                    if (stage.items) {
                        for (const item of stage.items) {
                            ...
                            doc.text(item.isOk ? '[ X ]' : '[   ]', startX + colWidths.si + colWidths.desc, currentY + 8, { width: colWidths.yes, align: 'center' });
                            doc.text('[   ]', startX + colWidths.si + colWidths.desc + colWidths.yes, currentY + 8, { width: colWidths.na, align: 'center' });
            ...
            const sigs = inspection.stages?.flatMap(s => s.signatures || []) || [];
