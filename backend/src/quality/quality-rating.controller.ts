...
export class QualityRatingController {
    ...
    @Roles('Admin')
    updateConfig(@Param('projectId', ParseIntPipe) projectId: number, @Body() body: any) {
        return this.service.updateConfig(projectId, body);
    }

    @Get(':projectId/calculate')
    calculate(@Param('projectId', ParseIntPipe) projectId: number, @Query('status') status: string) {
        return this.service.calculateProjectRating(projectId, status || 'Structure');
    }

    @Post(':projectId/snapshot')
    @Roles('Admin')
    createSnapshot(@Param('projectId', ParseIntPipe) projectId: number, @Body('status') status: string) {
