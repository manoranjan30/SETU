...
export class QualityWorkflowService {
    ...
    private readonly SNAG_WORKFLOW = {
        ...
        [QualityStatus.RECTIFIED]: {
            next: [QualityStatus.VERIFICATION_PENDING, QualityStatus.VERIFIED, QualityStatus.REJECTED],
    ...
    validateTransition(
        ...
        if (!rules) {
            throw new BadRequestException(`No rules defined for status ${item.status}`);
        }
