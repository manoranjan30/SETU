...
export class ApprovalWorkflowService {
  ...
  async getWorkflowByProjectId(
    ...
    if (!template) {
      throw new NotFoundException(
        `No global workflow found for project ${projectId}`,
      );
    }
  ...
  async saveWorkflow(
    ...
    try {
      let template = await queryRunner.manager.findOne(
      ...
      if (!template) {
        ...
        await queryRunner.manager.delete(ApprovalWorkflowEdge, {
          workflowId: template.id,
        });
        await queryRunner.manager.delete(ApprovalWorkflowNode, {
          workflowId: template.id,
        });
      ...
      for (const edgeData of data.edges) {
        ...
        if (!sourceId || !targetId) {
          throw new BadRequestException(
            'Invalid edge connecting to a non-existent node.',
          );
        }
  ...
  async deleteWorkflow(projectId: number): Promise<void> {
    ...
    if (!template) {
      throw new NotFoundException(
        `No global workflow template found for project ${projectId}`,
      );
    }
