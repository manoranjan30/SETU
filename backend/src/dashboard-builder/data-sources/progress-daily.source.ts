...
export class ProgressDailySource implements IDataSource {
    ...
    fields: DataSourceField[] = [
        ...
        { key: 'measuredQty', label: 'Measured Qty', type: 'number', aggregatable: true },
        { key: 'totalToDate', label: 'Total To Date', type: 'number', aggregatable: true },
    ...
    async execute(config: QueryConfig): Promise<any[]> {
        ...
        if (config.filters?.status?.length) {
            qb.andWhere('p.status IN (:...statuses)', { statuses: config.filters.status });
        }
