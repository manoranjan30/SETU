...
export class ProjectPortfolioSource implements IDataSource {
    ...
    fields: DataSourceField[] = [
        { key: 'projectName', label: 'Project Name', type: 'string', filterable: true },
        { key: 'projectCode', label: 'Project Code', type: 'string', filterable: true },
        { key: 'projectType', label: 'Type', type: 'string', groupable: true, filterable: true },
        { key: 'projectStatus', label: 'Status', type: 'string', groupable: true, filterable: true },
        ...
        { key: 'estimatedProjectCost', label: 'Estimated Cost', type: 'number', aggregatable: true },
        { key: 'approvedBudget', label: 'Approved Budget', type: 'number', aggregatable: true },
        { key: 'totalBuiltupArea', label: 'Total Built-up Area', type: 'number', aggregatable: true },
    ];

    filters: DataSourceFilter[] = [
        {
            key: 'projectStatus', label: 'Status', type: 'multi_select', options: [
        ...
        {
            key: 'projectType', label: 'Type', type: 'multi_select', options: [
    ...
    async execute(config: QueryConfig): Promise<any[]> {
        const qb = this.epsRepo.createQueryBuilder('e')
        ...
        if (config.filters?.projectStatus?.length) {
            qb.andWhere('pp.projectStatus IN (:...statuses)', { statuses: config.filters.projectStatus });
        }

        if (config.filters?.projectType?.length) {
            qb.andWhere('pp.projectType IN (:...types)', { types: config.filters.projectType });
        }
